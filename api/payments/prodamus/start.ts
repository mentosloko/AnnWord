...
const plans: Record<string, { code: string; title: string; amountRub: number; periodDays: number }> = {
  kids_month: { code: 'kids_month', title: 'AnnWord Kids Premium — 1 месяц', amountRub: 300, periodDays: 31 },
  kids_year: { code: 'kids_year', title: 'AnnWord Kids Premium — 1 год', amountRub: 3000, periodDays: 365 },
};
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const payload: Record<string, any> = { order_id: orderId, customer_email: email, products: [{ name: plan.title, price: plan.amountRub, quantity: 1 }], urlSuccess: `${origin}/payment/success?order_id=${encodeURIComponent(orderId)}`, urlReturn: `${origin}/payment/fail?order_id=${encodeURIComponent(orderId)}`, urlNotification: `${origin}/api/payments/prodamus/notify`, sys: 'annword', currency: 'rub' };
    payload.signature = sign(payload, required('PRODAMUS_SECRET_KEY'));
    const checkoutUrl = `${payformUrl()}/?${query(payload)}`;
    const { error: insertError } = await supabase.from('premium_payments').insert({ user_id: userData.user.id, provider: 'prodamus', provider_order_id: orderId, plan_code: plan.code, period_days: plan.periodDays, amount_rub: plan.amountRub, currency: 'RUB', status: 'pending', checkout_url: checkoutUrl, customer_email: email, raw_payload: { checkout: payload } });
    if (insertError) throw insertError;
    return res.status(200).json({ orderId, checkoutUrl, plan: { code: plan.code, title: plan.title, amountRub: plan.amountRub, periodDays: plan.periodDays } });
  } catch (error: unknown) {
    console.error('Prodamus start failed', error);
    return res.status(500).json({ error: error instanceof Error ? error.message : 'Payment start failed' });
  }
}
