...
export const AnagramGame: React.FC<AnagramGameProps> = ({ onBack, userProfile, onGameReward, onRecordReviewWord }) => {
  ...
  useEffect(() => {
    if (status === 'finished') { clearStoredGameSession(sessionKey(userProfile.username), legacySessionKey(userProfile.username)); return; }
    writeStoredGameSession(sessionKey(userProfile.username), { solvedCount, skippedCount, coinsEarned, activeWord: currentWord?.word, shuffledLetters: shuffledLetters.map(slot => slot.char), userGuess });
  ...
  const checkGuess = (guessLetters: GuessLetter[]) => {
    ...
    if (guess === currentWord.word) {
      ...
      setStatus('success');
      setMessage(earnedCoin ? `Правильно! За ${nextSolvedCount} слов получена 1 монета.` : `Правильно! Всего слов: ${nextSolvedCount}`);
      ...
      clearNextWordTimeout();
      nextWordTimeoutRef.current = setTimeout(() => { isCheckingRef.current = false; setStatus('playing'); setMessage(''); setShuffledLetters(previous => previous.map(slot => ({ ...slot, isUsed: false }))); setUserGuess([]); }, 1000);
    }
  };
