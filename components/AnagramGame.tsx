  ...
  const checkGuess = (guessLetters: GuessLetter[]) => {
    ...
    if (guess === currentWord.word) {
      ...
      nextWordTimeoutRef.current = setTimeout(() => {
        ...
        setStatus('playing');
        setMessage('');
        setShuffledLetters(previous => previous.map(slot => ({ ...slot, isUsed: false })));
        setUserGuess([]);
      }, 1000);
    }
  };
  ...
  return (
    <div className="flex w-full max-w-md flex-col items-center rounded-3xl bg-white p-3 shadow-xl sm:p-6">
      <div className="mb-4 flex w-full items-center justify-between gap-2 sm:mb-5">
        ...
        <div className="flex gap-1.5 text-xs font-black sm:gap-2"><span className="rounded-full bg-indigo-50 px-2.5 py-2 text-indigo-700 sm:px-3">⭐ {score}</span><span className="rounded-full bg-amber-50 px-2.5 py-2 text-amber-700 sm:px-3">₽ {coinsEarned}</span></div>
      </div>
      ...
      <div className="mb-5 grid min-h-[3.25rem] w-full grid-flow-col auto-cols-fr items-center gap-1.5 rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50 p-2 sm:mb-7 sm:min-h-[60px] sm:gap-2 sm:p-4">
        <AnimatePresence mode="popLayout">{userGuess.map((item, index) => <motion.button key={`${index}-${item.char}`} layout initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} onClick={() => handleGuessClick(index)} className="flex aspect-square w-full min-w-0 items-center justify-center rounded-lg border-2 border-indigo-500 bg-white text-[clamp(1rem,5.5vw,1.25rem)] font-bold text-indigo-600 shadow-sm">{item.char}</motion.button>)}</AnimatePresence>
      </div>
