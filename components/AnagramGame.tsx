...
import { buildPlayableGameDictionary, clearStoredGameSession, pickAdaptiveSessionWord, readStoredGameSession, updateReviewPriorities, WordPracticeResult, writeStoredGameSession } from '../services/gameSessionEngine';
...
export const AnagramGame: React.FC<AnagramGameProps> = ({ onBack, userProfile, onGameReward, onWordPractice }) => {
  ...
  const pickNewWord = useCallback(() => { if (dictionary.length === 0) return; isCheckingRef.current = false; const word = pickAdaptiveSessionWord('anagram', dictionary, reviewPriorities, currentWord?.word) || dictionary[Math.floor(Math.random() * dictionary.length)]; setCurrentWord(word); setShuffledLetters(shuffle(word.word.split('')).map((char, index) => ({ char, isUsed: false, originalIndex: index }))); setUserGuess([]); setStatus('playing'); setMessage(''); }, [currentWord?.word, dictionary, reviewPriorities, shuffle]);
  ...
  const checkGuess = (guessLetters: GuessLetter[]) => {
    ...
    if (guess === solvedWord) { const nextSolvedCount = solvedCount + 1; const earnedCoin = nextSolvedCount % 15 === 0 ? 1 : 0; setSolvedCount(nextSolvedCount); if (earnedCoin) setCoinsEarned(previous => previous + 1); setStatus('success'); setMessage(earnedCoin ? `Правильно! За ${nextSolvedCount} слов получена 1 монета.` : `Правильно! Всего слов: ${nextSolvedCount}`); void Promise.resolve(onGameReward({ type: 'anagram', guessedWords: 1, coinsAdjustment: earnedCoin })).then(() => onWordPractice?.(solvedWord, 'mastered')).catch(error => console.error('Failed to apply anagram success', error)); setReviewPriorities(previous => updateReviewPriorities(previous, solvedWord, 'mastered')); clearNextWordTimeout(); nextWordTimeoutRef.current = setTimeout(pickNewWord, 900); }
    else { registerPractice(solvedWord, 'failed'); setStatus('error'); setMessage('Неверно, попробуйте ещё раз!'); clearNextWordTimeout(); nextWordTimeoutRef.current = setTimeout(() => { isCheckingRef.current = false; setStatus('playing'); setMessage(''); setShuffledLetters(previous => previous.map(slot => ({ ...slot, isUsed: false }))); setUserGuess([]); }, 1000); }
  };
  ...
  const skipWord = () => { if (!currentWord || isCheckingRef.current || status !== 'playing') return; isCheckingRef.current = true; setStatus('checking'); setSkippedCount(previous => previous + 1); registerPractice(currentWord.word, 'failed'); setMessage(`Слово ${currentWord.word} добавлено для повторения. −1 балл.`); clearNextWordTimeout(); nextWordTimeoutRef.current = setTimeout(pickNewWord, 850); };
  const finishSession = () => { clearNextWordTimeout(); if (!sessionStatsAppliedRef.current && (solvedCount > 0 || skippedCount > 0)) { sessionStatsAppliedRef.current = true; void Promise.resolve(onGameReward({ type: 'anagram', guessedWords: solvedCount, statsOnly: true, wonForStats: solvedCount > 0 })).catch(error => console.error('Failed to save anagram session stats', error)); } setStatus('finished'); };
  const restartSession = () => { sessionStatsAppliedRef.current = false; setSolvedCount(0); setSkippedCount(0); setCoinsEarned(0); setCurrentWord(null); setStatus('playing'); setMessage(''); };
  ...
  return (
    <div className="flex w-full max-w-md flex-col items-center rounded-3xl bg-white p-3 shadow-xl sm:p-6">
      <div className="mb-4 flex w-full items-center justify-between gap-2 sm:mb-5"><button type="button" onClick={finishSession} className="rounded-xl bg-indigo-50 px-2.5 py-2 text-xs font-black text-indigo-700 sm:px-3 sm:text-sm">Закончить игру</button><div className="flex gap-1.5 text-xs font-black sm:gap-2"><span className="rounded-full bg-indigo-50 px-2.5 py-2 text-indigo-700 sm:px-3">⭐ {score}</span><span className="rounded-full bg-amber-50 px-2.5 py-2 text-amber-700 sm:px-3">Монеты: {coinsEarned}</span></div></div>
      <div className="mb-5 text-center sm:mb-7"><div className="mb-1 text-sm uppercase tracking-tighter text-gray-400">Перевод</div><div className="text-xl font-bold text-indigo-900 sm:text-2xl">{currentWord?.translation}</div></div>
      <div className="mb-5 grid w-full items-center gap-1.5 rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50 p-2 sm:mb-7 sm:gap-2 sm:p-4" style={{ gridTemplateColumns: `repeat(${activeWordLength}, minmax(0, 1fr))` }}>
        {Array.from({ length: activeWordLength }).map((_, index) => { const item = userGuess[index]; return <div key={`answer-slot-${index}`} className="relative aspect-square min-w-0 rounded-lg border-2 border-indigo-100 bg-white/70"><AnimatePresence>{item && <motion.button key={`${item.slotIndex}-${item.char}`} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} onClick={() => handleGuessClick(index)} className="absolute inset-0 flex items-center justify-center rounded-lg border-2 border-indigo-500 bg-white text-[clamp(1rem,5.5vw,1.25rem)] font-bold text-indigo-600 shadow-sm">{item.char}</motion.button>}</AnimatePresence></div>; })}
      </div>
