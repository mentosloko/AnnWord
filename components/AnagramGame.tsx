    ...
    <div className="flex w-full max-w-md flex-col items-center rounded-3xl bg-white p-3 shadow-xl sm:p-6">
      ...
      <div className="mb-5 grid w-full items-center gap-1.5 rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50 p-2 sm:mb-7 sm:gap-2 sm:p-4" style={{ gridTemplateColumns: `repeat(${activeWordLength}, minmax(0, 1fr))` }}>
        {Array.from({ length: activeWordLength }).map((_, index) => {
          ...
          return (
            <div key={`answer-slot-${index}`} className="relative aspect-square min-w-0 rounded-lg border-2 border-indigo-100 bg-white/70">
              <AnimatePresence>{item && <motion.button key={`${item.slotIndex}-${item.char}`} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} onClick={() => handleGuessClick(index)} className="absolute inset-0 flex items-center justify-center rounded-lg border-2 border-indigo-500 bg-white text-[clamp(1rem,5.5vw,1.25rem)] font-bold text-indigo-600 shadow-sm">{item.char}</motion.button>}</AnimatePresence>
            </div>
          );
        })}
      </div>
