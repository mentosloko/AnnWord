...
const makeRound = (pool: EnrichedWord[], previous?: string | null, review: Record<string, number> = {}, excluded: ReadonlySet<string> = new Set()): Round | null => {
  const available = pool.filter(item => !excluded.has(item.word));
  const word = pickAdaptiveSessionWord('letterSquare', available, review, previous) || random(available);
  if (!word) return null;
...
export const LetterSquareGameV3: React.FC<Props> = ({ onBack, userProfile, onGameReward, onWordPractice }) => {
  ...
  const next = useCallback((previous?: string) => {
    ...
    usedWordsRef.current.add(nextRound.word.word);
    setRound(nextRound);
    setSelected([]);
    setFeedback(null);
    setHint(false);
    setMessage('Соберите слово из соседних клеток. Диагонали нельзя.');
  }, [dictionary, review]);
