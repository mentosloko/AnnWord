...
const getCoinProgress = (score: number) => score >= 6 ? { label: 'Максимальный бонус монет', next: 6, progress: 100 } : score >= 3 ? { label: `Ещё ${6 - score} до большого бонуса`, next: 6, progress: Math.round(score / 6 * 100) } : { label: `Ещё ${3 - score} до монет`, next: 3, progress: Math.round(score / 3 * 100) };
...
export const SprintGame: React.FC<SprintGameProps> = ({ onBack, userProfile, onGameReward, onWordPractice, paused = false }) => {
  ...
  if (status === 'ended') return <GameResultOverlay isOpen status="completed" title="Спринт завершён" subtitle={`Отгадано слов: ${score}. ${rewardPreview.coins > 0 ? `Монеты: +${rewardPreview.coins}.` : 'До монет нужно минимум 3 правильных ответа.'}`} emoji="🏆" pet={progressPreview.pet} xpGained={rewardPreview.xp} coinsGained={rewardPreview.coins} onPrimary={restartGame} onSecondary={onBack} />;
  ...
  return (
    <div className="relative flex h-full min-h-0 w-full max-w-md flex-col items-center overflow-hidden rounded-3xl bg-white p-3 shadow-xl sm:h-auto sm:p-6" aria-busy={paused}>
      ...
      <div className="mb-2 mt-2 flex w-full shrink-0 flex-wrap items-center justify-between gap-2 sm:mb-4 sm:mt-4"><button onClick={onBack} className="flex min-h-[2.55rem] items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm font-bold text-gray-500 transition hover:text-indigo-600 sm:text-base"><span className="text-lg sm:text-xl" aria-hidden="true">←</span> Меню</button><div className="flex flex-wrap items-center justify-end gap-1.5 sm:gap-3"><div className="flex min-h-[2.4rem] items-center gap-1 rounded-full bg-gray-100 px-2 py-1 sm:gap-2 sm:px-3"><span className="text-base sm:text-xl" aria-hidden="true">⏱️</span><span className={`font-mono text-base font-bold sm:text-xl ${timeLeft < 10 ? 'animate-pulse text-red-500' : 'text-gray-700'}`}>{timeLeft}с</span></div><div className="flex min-h-[2.4rem] items-center gap-1 rounded-full bg-indigo-50 px-2 py-1 sm:gap-2 sm:px-3"><span className="text-base sm:text-xl" aria-hidden="true">⭐</span><span className="text-base font-bold text-indigo-600 sm:text-xl">{score}</span></div></div></div>
      <div className="mb-3 w-full rounded-2xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800"><div className="flex items-center justify-between gap-2"><span>{coinProgress.label}</span><span className="inline-flex items-center gap-1">{rewardPreview.coins}<CoinIcon /></span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-amber-100"><div className="h-full rounded-full bg-amber-500" style={{ width: `${coinProgress.progress}%` }} /></div></div>
      ...
      <AnimatePresence mode="wait"><motion.div key={question?.id || 'empty'} initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -50, opacity: 0 }} className="mb-4 mt-1 shrink-0 text-center sm:mb-10"><div className="mb-1 text-xs font-bold uppercase tracking-widest text-gray-400 sm:mb-2 sm:text-sm">Как переводится?</div><div className="break-words text-[clamp(1.9rem,9vw,2.65rem)] font-black tracking-tight text-indigo-900 sm:text-5xl">{question?.word.word}</div></motion.div></AnimatePresence>
