...
export const SprintGame: React.FC<SprintGameProps> = ({ onBack, userProfile, onGameReward }) => {
  ...
  return (
    <div className="relative flex w-full max-w-md flex-col items-center overflow-hidden rounded-3xl bg-white p-4 shadow-xl sm:p-6">
      ...
      <div className="mb-6 mt-4 flex w-full items-center justify-between gap-3"><button onClick={onBack} className="flex items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1 font-bold text-gray-500 transition hover:text-indigo-600"><span className="text-xl">←</span> Меню</button><div className="flex items-center gap-2 sm:gap-4"><div className="flex items-center gap-2 rounded-full bg-gray-100 px-3 py-1"><span className="text-xl">⏱️</span><span className={`font-mono text-xl font-bold ${timeLeft < 10 ? 'animate-pulse text-red-500' : 'text-gray-700'}`}>{timeLeft}с</span></div><div className="flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1"><span className="text-xl">⭐</span><span className="text-xl font-bold text-indigo-600">{score}</span></div></div></div>
