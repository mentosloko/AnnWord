...
interface SprintGameProps { onBack: () => void; userProfile: UserProfile; onGameReward: (input: GameRewardInput) => void | Promise<void>; onWordPractice?: (word: string, result: WordPracticeResult) => void | Promise<void>; paused?: boolean; }
interface SprintQuestion { id: number; word: EnrichedWord; options: string[]; correctAnswer: string; }
export const buildSprintDictionary = (customDictionaryEn: string[] = [], fallbackDictionary: EnrichedWord[] = COMMON_WORDS_EN): EnrichedWord[] => buildPlayableGameDictionary(customDictionaryEn, fallbackDictionary);
...
export const SprintGame: React.FC<SprintGameProps> = ({ onBack, userProfile, onGameReward, onWordPractice, paused = false }) => {
  ...
  const coinProgress = getCoinProgress(score);
  useEffect(() => { latestDictionaryRef.current = dictionary; if (activeDictionaryRef.current.length === 0 && dictionary.length > 0) activeDictionaryRef.current = dictionary; }, [dictionary]);
  ...
  useEffect(() => {
    if (status !== 'ended' || rewardAppliedRef.current) return;
    rewardAppliedRef.current = true;
    const reward = calculateGameReward({ type: 'sprint', guessedWords: score });
    setResultProgress(showKidsRewards ? applyGameRewardToCharacter(userProfile.pet, reward) : null);
    void Promise.resolve(onGameReward({ type: 'sprint', guessedWords: score })).catch(error => console.error('Failed to save Sprint result', error));
  }, [status, score, onGameReward, showKidsRewards, userProfile.pet]);
  const rewardPreview = calculateGameReward({ type: 'sprint', guessedWords: score });
  const handleOptionClick = (option: string) => { if (status !== 'playing' || feedback || !question || paused) return; clearNextWordTimeout(); if (option === question.correctAnswer) { setScore(previous => previous + 1); setFeedback('correct'); registerPractice(question.word.word, 'mastered'); nextWordTimeoutRef.current = setTimeout(pickNewWord, 500); } else { setFeedback('wrong'); registerPractice(question.word.word, 'failed'); nextWordTimeoutRef.current = setTimeout(pickNewWord, 800); } };
  ...
  if (status === 'ended') return <GameResultOverlay isOpen status="completed" title="Спринт завершён" subtitle={showKidsRewards ? `Звёзд: ${score}. ${rewardPreview.coins > 0 ? `Монеты: +${rewardPreview.coins}.` : 'До монет нужно минимум 6 звёзд.'}` : `Звёзд: ${score}. Слова добавлены в статистику практики.`} emoji="🏆" pet={resultProgress?.pet} xpGained={showKidsRewards ? rewardPreview.xp : 0} coinsGained={showKidsRewards ? rewardPreview.coins : 0} onPrimary={restartGame} onSecondary={onBack} />;
  return <div className="relative flex h-full min-h-0 w-full max-w-md flex-col items-center overflow-hidden rounded-3xl bg-white p-3 shadow-xl sm:h-auto sm:p-6" aria-busy={paused}><div className="absolute left-0 top-0 h-2 w-full bg-indigo-100"><motion.div className="h-full bg-indigo-600" initial={{ width: '100%' }} animate={{ width: `${(timeLeft / 60) * 100}%` }} transition={{ duration: paused ? 0 : 1, ease: 'linear' }} /></div><div className="mb-2 mt-2 flex w-full shrink-0 flex-wrap items-center justify-between gap-2 sm:mb-4 sm:mt-4"><button onClick={onBack} className="flex min-h-[2.55rem] items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm font-bold text-gray-500 transition hover:text-indigo-600 sm:text-base"><span className="text-lg sm:text-xl" aria-hidden="true">←</span> Меню</button><div className="flex flex-wrap items-center justify-end gap-1.5 sm:gap-3"><div className="flex min-h-[2.4rem] items-center gap-1 rounded-full bg-gray-100 px-2 py-1 sm:gap-2 sm:px-3"><span className="text-base sm:text-xl" aria-hidden="true">⏱️</span><span className={`font-mono text-base font-bold sm:text-xl ${timeLeft < 10 ? 'animate-pulse text-red-500' : 'text-gray-700'}`}>{timeLeft}с</span></div><div className="flex min-h-[2.4rem] items-center gap-1 rounded-full bg-indigo-50 px-2 py-1 sm:gap-2 sm:px-3"><span className="text-base sm:text-xl" aria-hidden="true">⭐</span><span className="text-base font-bold text-indigo-600 sm:text-xl">{score}</span></div></div></div>{showKidsRewards && <div className="mb-3 w-full rounded-2xl bg-amber-50 px-3 py-2 text-xs font-black text-amber-800"><div className="flex items-center justify-between gap-2"><span>{coinProgress.label}</span><span className="inline-flex items-center gap-1">{rewardPreview.coins}<CoinIcon /></span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-amber-100"><div className="h-full rounded-full bg-amber-500" style={{ width: `${coinProgress.progress}%` }} /></div></div>}{paused && <div className="mb-2 rounded-full bg-indigo-50 px-3 py-1 text-xs font-black uppercase tracking-widest text-indigo-500">Пауза</div>}<AnimatePresence mode="wait"><motion.div key={question?.id || 'empty'} initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -50, opacity: 0 }} className="mb-4 mt-1 shrink-0 text-center sm:mb-10"><div className="mb-1 text-xs font-bold uppercase tracking-widest text-gray-400 sm:mb-2 sm:text-sm">Как переводится?</div><div className="break-words text-[clamp(1.9rem,9vw,2.65rem)] font-black tracking-tight text-indigo-900 sm:text-5xl">{question?.word.word}</div></motion.div></AnimatePresence><div className="grid min-h-0 w-full flex-1 grid-cols-1 content-center gap-2 sm:flex-none sm:gap-3">{question?.options.map((option, index) => <motion.button key={`${question.id}-${option}-${index}`} aria-label={`Ответ: ${option}`} whileHover={{ scale: paused ? 1 : 1.02 }} whileTap={{ scale: paused ? 1 : 0.98 }} disabled={paused || status !== 'playing'} onClick={() => handleOptionClick(option)} className={`flex w-full min-h-[2.8rem] items-center justify-between rounded-2xl border-2 px-4 py-[clamp(0.65rem,1.85dvh,1.05rem)] text-sm font-bold transition-all disabled:opacity-60 sm:px-6 sm:text-lg ${feedback === 'correct' && option === question.correctAnswer ? 'border-green-600 bg-green-500 text-white shadow-lg' : feedback === 'wrong' && option !== question.correctAnswer ? 'border-gray-100 bg-white text-gray-400 opacity-50' : feedback === 'wrong' && option === question.correctAnswer ? 'border-green-500 bg-green-100 text-green-700' : 'border-gray-100 bg-white text-gray-700 shadow-sm hover:border-indigo-200 hover:bg-indigo-50'}`}><span>{option}</span>{feedback === 'correct' && option === question.correctAnswer && <span aria-hidden="true">✅</span>}</motion.button>)}</div></div>;
};