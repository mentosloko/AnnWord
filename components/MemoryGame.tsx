...
export const MemoryGame: React.FC<MemoryGameProps> = ({ onBack, userProfile, onGameReward }) => {
  ...
  const handleCardClick = (id: number) => {
    ...
    if (newFlipped.length === 2) {
      ...
      if (firstCard?.pairId === secondCard?.pairId) setTimeout(() => { setCards(previous => previous.map(card => card.pairId === firstCard?.pairId ? { ...card, isMatched: true } : card)); setFlippedCards([]); }, 500);
      else setTimeout(() => { setCards(previous => previous.map(card => (card.id === firstId || card.id === secondId) ? { ...card, isFlipped: false } : card)); setFlippedCards([]); }, 1000);
    }
  };
