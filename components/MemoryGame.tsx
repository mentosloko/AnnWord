...
interface MemoryGameProps { onBack: () => void; userProfile: UserProfile; onGameReward: (input: GameRewardInput) => void | Promise<void>; onWordPractice?: (word: string, result: WordPracticeResult) => void | Promise<void>; }
interface Card { id: number; content: string; word: string; type: 'en' | 'ru'; pairId: number; isFlipped: boolean; isMatched: boolean; }
...
const createMemoryCards = (dictionary: EnrichedWord[]): Card[] => shuffle(shuffle(dictionary).slice(0, Math.min(6, dictionary.length)).flatMap((word, pairId) => [
  { id: pairId * 2, content: word.word, word: word.word, type: 'en' as const, pairId, isFlipped: false, isMatched: false },
  { id: pairId * 2 + 1, content: word.translation, word: word.word, type: 'ru' as const, pairId, isFlipped: false, isMatched: false },
]));

export const MemoryGame: React.FC<MemoryGameProps> = ({ onBack, userProfile, onGameReward, onWordPractice }) => {
  ...
  const handleCardClick = (id: number) => {
    ...
    if (newFlipped.length === 2) {
      ...
      if (firstCard?.pairId === secondCard?.pairId) setTimeout(() => { if (firstCard?.word) registerPractice(firstCard.word, 'mastered'); setCards(previous => previous.map(card => card.pairId === firstCard?.pairId ? { ...card, isMatched: true } : card)); setFlippedCards([]); }, 550);
      else setTimeout(() => { if (firstCard?.word) registerPractice(firstCard.word, 'failed'); if (secondCard?.word && secondCard.word !== firstCard?.word) registerPractice(secondCard.word, 'failed'); setCards(previous => previous.map(card => (card.id === firstId || card.id === secondId) ? { ...card, isFlipped: false } : card)); setFlippedCards([]); }, 1200);
    }
  };
  useEffect(() => { if (cards.length > 0 && cards.every(card => card.isMatched) && !isWon) setIsWon(true); }, [cards, isWon]);
  useEffect(() => { if (isWon && !rewardAppliedRef.current) { rewardAppliedRef.current = true; void Promise.resolve(onGameReward({ type: 'memory', clicks })).catch(error => console.error('Failed to save memory reward', error)); } }, [isWon, clicks, onGameReward]);
  const rewardPreview = calculateGameReward({ type: 'memory', clicks });
  const progressPreview = showKidsRewards ? applyGameRewardToCharacter(userProfile.pet, rewardPreview) : null;
