...
export const AppScreens: React.FC<AppScreensProps> = ({ route, entryPath, userProfile, isAuthenticated, sessionOwnerId, dailyQuest, dailyQuestReward, onCloseDailyQuestReward, settings, modeWords, activeDictionaryWordCount, selectedPlayMode, classicGame, dictionaryUpload, onRouteChange, onEntryPathChange, onSelectedPlayModeChange, onSettingsChange, onOpenLogin, onOpenRegister, onBuy, onUseItem, onUpdatePet, onSaveDictionary, onSelectAccountMode, onCreateChild, onChildSetupComplete, onGameReward, onWordPractice, onCharacterOnboardingComplete, onGameStarted, onTestUnlockPremium, onDictionaryPeek }) => {
  ...
  const screens: Partial<Record<ViewState, React.ReactNode>> = {
    ...
    letter_square: <GameModeShell gameId="letter_square" viewerKey={rulesViewerKey} title="Змейка" subtitle="Соединяй буквы цепочкой" rules={letterSquareRules} showDictionary={false} onBackHome={goHome}><LetterSquareGameV3 key={`${playDictionaryId}:${playWords.join('|')}`} userProfile={{ ...userProfile, customDictionaryEn: playWords }} sessionOwnerId={sessionOwnerId} dictionaryId={playDictionaryId} dictionaryLabel={playDictionaryName} dictionaryIcon={playDictionaryIcon} onGameReward={onGameReward} onWordPractice={onWordPractice} onHintCharge={onDictionaryPeek} onBack={goHome} /></GameModeShell>,
    shop: isParentAccount ? <Shop userProfile={userProfile} onBuy={onBuy} onClose={goHome} onOpenPetRoom={() => onRouteChange('pet_room')} onPlay={() => onRouteChange('setup')} /> : homeScreen,
    pet_room: isParentAccount ? <div className="h-[100dvh] min-h-[100svh] overflow-y-auto overscroll-contain"><PetRoom userProfile={userProfile} onUseItem={onUseItem} onBuy={onBuy} onUpdatePet={onUpdatePet} onClose={goHome} onOpenShop={() => onRouteChange('shop')} onPlay={() => onRouteChange('setup')} /></div> : homeScreen,
  };
