...
export const AppHeader: React.FC<AppHeaderProps> = ({ route, userProfile, isAuthenticated, onHomeClick, onLoginClick, onLogoutClick, onProfileClick, onShopClick, onAdminClick, onAdultRoomClick, onDictionaryStudioClick }) => {
  ...
  return <header className="sticky top-0 z-40 flex w-full items-center justify-between gap-2 border-b border-indigo-50 bg-white/85 px-3 py-2.5 backdrop-blur sm:gap-3 sm:px-4 sm:py-3">
    <div className="flex min-w-0 items-center gap-2"><button type="button" onClick={onHomeClick} aria-label="AnnWord — на главную" className="flex min-w-0 items-center gap-2 rounded-xl transition hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 sm:gap-2.5"><img src="/assets/branding/annword-logo-mark.svg" alt="" aria-hidden="true" className="h-10 w-10 shrink-0 object-contain sm:h-11 sm:w-11" draggable={false}/><span className="truncate text-lg font-black leading-none text-indigo-950 sm:text-xl">AnnWord</span></button>{guestParentLanding && <span className="hidden rounded-md bg-gradient-to-r from-violet-600 to-indigo-600 px-2 py-1 text-xs font-black lowercase text-white shadow-sm sm:inline-flex">kids</span>}{modeLabel !== 'AnnWord' && <span className="hidden rounded-full bg-indigo-50 px-3 py-1 text-xs font-black text-indigo-600 sm:inline-flex">{modeLabel}</span>}</div>
    ...
    {guestParentLanding && <nav className="hidden items-center gap-1 lg:flex" aria-label="Навигация по AnnWord kids"><a href="/#how-it-works" className={navButton + ' text-indigo-950'}>Как это работает</a><a href="/#game-modes" className={navButton + ' text-indigo-950'}>Режимы игры</a><a href="/#for-parents" className={navButton + ' text-indigo-950'}>Для родителей</a><a href="/premium" className={navButton + ' text-indigo-950'}>Тарифы</a></nav>}
    <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
      ...
      {guestParentLanding && <button type="button" onClick={onLoginClick} className="hidden rounded-xl border-2 border-violet-200 bg-white px-4 py-2 text-sm font-black text-violet-700 transition hover:bg-violet-50 sm:inline-flex">Войти</button>}
      {guestParentLanding && <a href="/kids" className="rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2 text-sm font-black text-white shadow-lg shadow-indigo-600/20 transition hover:-translate-y-0.5 sm:px-5">Создать кидс-профиль</a>}
