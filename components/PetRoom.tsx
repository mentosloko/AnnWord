...
export const PetRoom: React.FC<Props> = ({ userProfile, onUseItem, onBuy, onClose, onOpenShop }) => {
  ...
  const use = async (id: string) => {
    ...
    setFeedback({ title: currentItem?.type === 'accessory' ? 'Наряд обновлён' : 'Лакомство использовано', detail: currentItem?.type === 'accessory' ? `${currentItem.name} теперь на питомце.` : `${currentItem?.name || 'Лакомство'} · +${moodDelta} к настроению`, itemId: id });
  ...
  return <div className="mx-auto max-w-6xl px-3 pb-24 pt-3 sm:px-4">
    ...
    <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]"><div ref={roomViewportRef} className="overflow-x-auto rounded-[2rem] bg-white shadow-sm sm:overflow-hidden"><div className={`relative min-h-[420px] w-[660px] sm:min-h-[540px] sm:w-full ${room ? 'bg-cover bg-center' : `bg-gradient-to-b ${world.backgroundClass}`}`} style={room ? { backgroundImage: `url('${room}')` } : undefined}>{dailyWorld && <div className="absolute left-4 top-4 rounded-full bg-white/90 px-4 py-2 text-xs font-black text-indigo-700">{world.emoji} Фон дня: {world.title}</div>}<motion.button type="button" aria-label={`Погладить питомца ${pet.name}`} title={`Погладить питомца ${pet.name}`} onClick={() => setSpeech(say(profile))} animate={feedback ? { y: [0, -16, 0], rotate: [0, -3, 3, 0] } : { y: mood.mood === 'sad' ? [0, -4, 0] : [0, -10, 0] }} transition={{ repeat: Infinity, duration: feedback ? 1.2 : 3 }} className="absolute bottom-20 left-1/2 h-56 w-56 -translate-x-1/2 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-indigo-400 sm:h-72 sm:w-72">{picture ? <img src={picture} alt="" aria-hidden="true" className="h-full w-full object-contain" /> : <span className="text-8xl" aria-hidden="true">{getPetEmoji(pet)}</span>}</motion.button><div className="absolute right-4 top-4 max-w-[16rem] rounded-3xl bg-white/90 px-4 py-3 text-sm font-bold text-indigo-900">“{speech}”</div>{feedback && <motion.div initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute bottom-72 left-1/2 -translate-x-1/2 rounded-full bg-green-500 px-4 py-2 text-sm font-black text-white shadow-lg">{feedback.title}</motion.div>}</div></div>
