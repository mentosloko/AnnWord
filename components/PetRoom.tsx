...
export const PetRoom: React.FC<Props> = ({ userProfile, onUseItem, onBuy, onClose, onOpenShop }) => {
  ...
  const accessories = flags.levelWardrobe ? getLevelAvailableAccessories(xp.level) : [];
  const nextAccessories = flags.levelWardrobe ? getShopItemsByType('accessory').filter(item => item.minLevel > xp.level).sort((a, b) => a.minLevel - b.minLevel).slice(0, 2) : [];
  ...
  const buy = async (item: ShopItem) => {
    ...
    setProfile(next.profile);
    setSpeech(wasWanted ? `Спасибо за «${item.name}»! Я очень рад!` : say(next.profile));
    setBusy(item.id);
