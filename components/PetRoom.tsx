  ...
  return (
    <div className="mx-auto max-w-6xl px-3 pb-24 pt-3 sm:px-4">
      ...
      <section className="mt-5 rounded-[2rem] border-2 border-indigo-50 bg-white p-3 sm:p-4">
        ...
        {items.length ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {items.map(item => {
              ...
              const effect = tab === 'food' ? treatEffect(item.id) : null;
              return <button type="button" key={item.id} onClick={() => void use(item.id)} aria-pressed={isEquipped} className={`relative flex items-center gap-3 rounded-3xl border-2 p-3 text-left transition ${isEquipped ? 'border-indigo-500 bg-indigo-50 ring-2 ring-indigo-100' : 'border-indigo-100 bg-white hover:bg-indigo-50/40'}`}>
                ...
                <div className={`min-w-0 font-black ${isEquipped ? 'text-indigo-700' : 'text-indigo-950'}`}><span className="block">{item.name}</span>{effect && <span className="mt-0.5 block text-[11px] font-bold text-green-600">{effect}</span>}{tab === 'food' && <span className="mt-1 inline-flex rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs text-indigo-700" aria-label={`Количество: ${item.quantity}`}>×{item.quantity}</span>}</div>
        ...
        {offers.length > 0 && <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{offers.map(item => { const image = getShopImageUrl(item); const effect = item.type === 'food' ? treatEffect(item.id) : null; return <div key={item.id} className="flex items-center gap-3 rounded-3xl border-2 border-dashed border-indigo-200 p-3"><div className="h-16 w-16">{image && <img src={image} alt="" className="h-full w-full object-contain" />}</div><div><div className="font-black">{item.name}</div>{effect && <div className="text-[11px] font-bold text-green-600">{effect}</div>}<div className="flex items-center gap-1 text-sm font-black text-yellow-700">{item.price}<CoinIcon /></div><button type="button" disabled={Boolean(busy)} onClick={() => void buy(item)} className="mt-2 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-black text-white disabled:bg-indigo-200">{busy === item.id ? 'Сохраняю...' : 'Купить'}</button></div></div>; })}</div>}
      </section>
    </div>
  );
};