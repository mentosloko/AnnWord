...
export const Keyboard: React.FC<KeyboardProps> = ({ onChar, onDelete, onEnter, letterStatuses }) => {
  const getKeyClass = (key: string) => {
    ...
    if (isControlKey) {
      base += 'h-[clamp(2.35rem,6.6dvh,3.4rem)] flex-[1.55] px-1 text-[clamp(0.74rem,2.7vw,1rem)] ';
      return base + 'bg-indigo-100 hover:bg-indigo-200 text-indigo-950 border-indigo-200';
    }

    base += 'h-[clamp(2.35rem,6.6dvh,3.4rem)] flex-1 text-[clamp(0.86rem,3.25vw,1.12rem)] ';
  ...
  return (
    <div className="w-full max-w-[min(40rem,100vw)] px-1 pb-[max(0.25rem,env(safe-area-inset-bottom))]">
      {KEYBOARD_LAYOUT_EN.map((row, i) => (
        <div
          key={i}
          className={`flex gap-[clamp(0.18rem,0.8vw,0.45rem)] mb-[clamp(0.18rem,0.75vw,0.42rem)] justify-center w-full ${i === 1 ? 'pl-[4.5%]' : i === 2 ? 'pl-[9%]' : ''}`}
        >
          {row.map(renderKey)}
        </div>
      ))}
