...
export const ParentContactEmailCard: React.FC = () => {
  ...
  return <section className="rounded-2xl border-2 border-sky-100 bg-sky-50 p-3" aria-labelledby="parent-contact-email-title">
    <div className="flex items-center justify-between gap-2">
      ...
      <span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${savedEmail ? 'bg-green-100 text-green-700' : 'bg-white text-slate-500'}`}>{savedEmail ? 'указан' : 'не указан'}</span>
    </div>
    ...
    {loading && <p className="mt-2 text-xs font-bold text-sky-700">Загружаю настройку…</p>}
