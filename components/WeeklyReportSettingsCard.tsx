...
import { ParentContactEmailCard } from './ParentContactEmailCard';
...
export const WeeklyReportSettingsCard: React.FC<Props> = ({ userProfile, premiumActive }) => {
  ...
  useEffect(() => {
    let active = true;
    void profileApiService.getParentContactEmail().then(result => {
      if (!active) return;
      const next = result.email || '';
      setParentEmail(next);
      if (!userProfile.weeklyReportEmail) setEmail(current => current || next);
    }).catch(() => undefined);
    const onParentEmailUpdated = (event: Event) => {
      const next = (event as CustomEvent<{ email?: string | null }>).detail?.email || '';
      setParentEmail(next);
      if (!savedEmail) setEmail(current => current || next);
    };
    window.addEventListener('annword:parent-email-updated', onParentEmailUpdated);
    return () => {
      active = false;
      window.removeEventListener('annword:parent-email-updated', onParentEmailUpdated);
    };
  }, [savedEmail, userProfile.weeklyReportEmail]);
  ...
  const reportCard = premiumChecking
    ? <div className="mt-4 rounded-2xl bg-indigo-50 p-3" role="status" aria-live="polite"><div className="text-xs font-black uppercase text-indigo-500">Еженедельный отчёт</div><p className="mt-2 text-sm font-bold text-indigo-700">Проверяем доступ и сохранённые настройки…</p></div>
    : !premiumActive
      ? <div className="mt-4 rounded-2xl bg-gray-50 p-3"><div className="text-xs font-black uppercase text-gray-400">Отчёт на почту</div><p className="mt-2 text-sm font-bold text-gray-500">Еженедельные отчёты доступны в Kids Premium. Сохранённый email родителя останется в профиле.</p></div>
      : (() => {
        ...
        return <section className="mt-4 rounded-2xl bg-indigo-50 p-3" aria-labelledby="weekly-report-title">
          ...
          {!savedEmail && parentEmail && email.trim().toLowerCase() === parentEmail.trim().toLowerCase() && <p className="mt-2 text-[11px] font-bold text-indigo-500">Подставлен контактный email родителя. Нажмите «Включить отчёт», чтобы подтвердить отправку.</p>}
  ...
  return <><ParentContactEmailCard />{reportCard}</>;
};
