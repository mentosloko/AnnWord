...
export const HangmanGame: React.FC<HangmanGameProps> = ({ onBack, userProfile, onGameReward }) => {
  ...
  const pickNewWord = useCallback(() => {
    ...
    setCurrentWord(word); setGuessedLetters([]); setMistakes(0); setFinalMistakes(0); setStatus('playing'); rewardAppliedRef.current = false;
  }, [dictionary]);
  ...
  const handleLetterClick = (rawLetter: string) => {
    ...
    const nextGuessedLetters = [...guessedLetters, letter]; setGuessedLetters(nextGuessedLetters);
    if (!currentWord.word.includes(letter)) setMistakes(previous => { const newMistakes = previous + 1; if (newMistakes >= maxMistakes) { setFinalMistakes(newMistakes); setStatus('lost'); } return newMistakes; });
    else if (currentWord.word.split('').every(character => nextGuessedLetters.includes(character))) { setFinalMistakes(mistakes); setStatus('won'); }
  };
  ...
  return (
    <div className="relative flex w-full max-w-md flex-col items-center overflow-hidden rounded-3xl bg-white p-4 shadow-xl sm:p-6">
      <div className="mb-8 flex w-full items-center justify-between"><button onClick={onBack} className="flex items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1 font-bold text-gray-500 transition hover:text-indigo-600"><span className="text-xl">←</span> Меню</button><div className="text-sm font-bold uppercase tracking-widest text-indigo-600">Угадай слово</div><div className="w-16" /></div>
      <div className="mb-8 w-full rounded-2xl bg-indigo-50 p-4"><div className="mb-2 flex justify-center gap-1 sm:gap-2">{Array.from({ length: maxMistakes }).map((_, index) => <motion.span key={`heart-${index}`} animate={{ opacity: index < maxMistakes - mistakes ? 1 : 0.3 }} className="text-2xl sm:text-3xl">❤️</motion.span>)}</div><div className="text-center text-xs font-bold uppercase tracking-widest text-indigo-400">Осталось попыток: {maxMistakes - mistakes}</div></div>
      <div className="mb-10 flex flex-wrap justify-center sm:mb-12">{currentWord?.word.split('').map((character, index) => <div key={index} className="mx-1 flex h-10 w-8 items-center justify-center border-b-4 border-indigo-600 text-2xl font-black text-indigo-900 sm:h-12 sm:w-10 sm:text-3xl">{guessedLetters.includes(character) || status === 'lost' ? character : ''}</div>)}</div>
      <div className="grid w-full grid-cols-7 gap-1.5 sm:gap-2">{alphabet.map(letter => <button key={letter} type="button" disabled={guessedLetters.includes(letter) || status !== 'playing'} onClick={() => handleLetterClick(letter)} className={`h-9 rounded-lg text-xs font-bold transition-all sm:h-10 sm:text-sm ${guessedLetters.includes(letter) ? currentWord?.word.includes(letter) ? 'border-2 border-green-200 bg-green-100 text-green-600' : 'border-2 border-gray-100 bg-gray-100 text-gray-300' : 'border-2 border-gray-100 bg-white text-gray-700 shadow-sm hover:border-indigo-400 hover:bg-indigo-50'}`}>{letter}</button>)}</div>
      {rewardPreview && progressPreview && <GameResultOverlay isOpen={status !== 'playing'} status={status === 'won' ? 'won' : 'lost'} title={status === 'won' ? 'Победа!' : 'Почти получилось'} subtitle={status === 'won' ? `Слово угадано. Осталось попыток: ${remainingAttempts}.` : 'Слово открылось — можно попробовать снова.'} emoji={status === 'won' ? '🎉' : '💪'} pet={progressPreview.pet} xpGained={rewardPreview.xp} coinsGained={rewardPreview.coins} onPrimary={pickNewWord} onSecondary={onBack} details={<span>Слово: <span className="font-black">{currentWord?.word}</span>{currentWord?.translation ? ` · ${currentWord.translation}` : ''}{status === 'won' && rewardPreview.coins > 0 ? ` · бонус за попытки: +${rewardPreview.coins} монет` : ''}</span>} />}
    </div>
  );
};