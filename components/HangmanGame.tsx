...
export const HangmanGame: React.FC<HangmanGameProps> = ({ onBack, userProfile, onGameReward }) => {
  ...
  return (
    <div className="relative flex w-full max-w-md flex-col items-center overflow-hidden rounded-3xl bg-white p-4 shadow-xl sm:p-6">
      ...
      <div className="mb-8 flex w-full items-center justify-between"><button type="button" onClick={onBack} className="flex items-center gap-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1 font-bold text-gray-500 transition hover:text-indigo-600"><span className="text-xl" aria-hidden="true">←</span> Меню</button><div className="text-sm font-bold uppercase tracking-widest text-indigo-600">Угадай слово</div><div className="w-16" /></div>
      <div className="mb-8 w-full rounded-2xl bg-indigo-50 p-4"><div className="mb-2 flex justify-center gap-1 sm:gap-2" aria-hidden="true">{Array.from({ length: maxMistakes }).map((_, index) => <motion.span key={`heart-${index}`} animate={{ opacity: index < maxMistakes - mistakes ? 1 : 0.3 }} className="text-2xl sm:text-3xl">❤️</motion.span>)}</div><div className="text-center text-xs font-bold uppercase tracking-widest text-indigo-400">Осталось попыток: {maxMistakes - mistakes}</div></div>
      <div className="mb-10 flex flex-wrap justify-center sm:mb-12" aria-label={`Слово из ${currentWord?.word.length || 0} букв`}>{currentWord?.word.split('').map((character, index) => <div key={index} aria-label={guessedLetters.includes(character) ? `Позиция ${index + 1}: ${character}` : `Позиция ${index + 1}: скрытая буква`} className="mx-1 flex h-10 w-8 items-center justify-center border-b-4 border-indigo-600 text-2xl font-black text-indigo-900 sm:h-12 sm:w-10 sm:text-3xl">{guessedLetters.includes(character) ? character : ''}</div>)}</div>
