...
const GameIconButton: React.FC<GameOption> = ({ title, iconSrc, onStart, hasSavedGame }) => (
  <button type="button" aria-label={title} onClick={onStart} className="group relative flex min-w-0 flex-col items-center rounded-2xl border-2 border-indigo-50 bg-white px-1.5 py-2.5 text-center shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg sm:rounded-3xl sm:px-2 sm:py-3">
    {hasSavedGame && <span aria-label="Есть сохранённая игра" title="Есть сохранённая игра" className="absolute -right-1 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-green-50 text-[11px] font-black text-green-700 shadow-sm sm:right-1 sm:top-1">💾</span>}
...
export const LandingScreen: React.FC<LandingScreenProps> = ({
  ...
  const gameOptions: GameOption[] = [
    { title: 'Классика', iconSrc: '/assets/games/game_classic.webp', onStart: onStartClassic, hasSavedGame: hasActiveClassicGame },
    { title: 'Анаграммы', iconSrc: '/assets/games/game_anagrams.webp', onStart: onStartAnagrams, hasSavedGame: hasActiveAnagramGame },
