...
const GameIconButton: React.FC<GameOption> = ({ title, iconSrc, onStart, saved }) => (
  <button
    type="button"
    aria-label={saved ? `${title}. Игра сохранена` : title}
    onClick={onStart}
  ...
  >
    {saved && <span aria-hidden="true" title="Игра сохранена" className="absolute -right-1 -top-2 rounded-full bg-green-50 px-1.5 py-0.5 text-[11px] font-black text-green-700 sm:right-1 sm:top-1">💾</span>}
...
export const LandingScreen: React.FC<LandingScreenProps> = ({
  ...
  const gameOptions: GameOption[] = [
    { title: 'Классика', iconSrc: '/assets/games/game_classic.webp', onStart: onStartClassic, saved: hasActiveClassicGame },
    { title: 'Анаграммы', iconSrc: '/assets/games/game_anagrams.webp', onStart: onStartAnagrams, saved: hasActiveAnagramGame },
  ...
  return (
    <ScreenContainer className="pb-6 sm:pb-24">
      <section className="grid grid-cols-1 items-center gap-5 py-4 sm:py-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-8">
        <div>
          {isAuthenticated && <><h1 className="mb-4 text-3xl font-black leading-tight text-indigo-950 sm:text-5xl">Во что сыграем сегодня?</h1><p className="mb-6 max-w-xl text-base leading-relaxed text-gray-600 sm:text-lg">Играйте и собирайте монеты. {userProfile.pet.name} будет расти вместе с вами.</p></>}
