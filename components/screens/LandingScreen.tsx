      ...
      <section className="grid grid-cols-1 items-center gap-5 py-4 sm:py-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-8">
        <div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-5 sm:gap-3">
            {gameOptions.map(game => <GameIconButton key={`${game.title}-${game.iconSrc}`} {...game} />)}
          </div>
        ...
        <aside className="rounded-[2rem] bg-gradient-to-br from-indigo-600 to-purple-700 p-5 text-white shadow-2xl sm:p-6">
          ...
          {!isAuthenticated && (
            <div className="rounded-3xl border border-white/10 bg-white/10 p-5">
              ...
              <p className="mb-4 text-sm leading-relaxed text-white/75">После входа будут сохраняться ₽, покупки, опыт и личный словарь.</p>
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={onOpenShop} className="flex min-h-[8.25rem] flex-col justify-between rounded-2xl border border-white/10 bg-white/10 p-4 text-left transition hover:bg-white/15">
                  <CoinIcon className="text-3xl" label="рубли" />
                  <div><div className="text-2xl font-black">{userProfile.coins}</div><div className="text-xs font-bold uppercase tracking-widest text-white/65">рублей</div></div>
                </button>
                <div className="flex min-h-[8.25rem] flex-col justify-between rounded-2xl border border-white/10 bg-white/10 p-3">
                  ...
                  <div>
                    ...
                    <div className="h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full bg-white" style={{ width: `${xpProgress}%` }} /></div></div>
                  </div>
                  <div>
                    <div className={`mb-1 flex justify-between text-[10px] font-black uppercase tracking-widest ${moodDisplay.textOnDarkClass}`}><span>{moodDisplay.label}</span><span>{moodScore}</span></div>
                    <div className="h-2 overflow-hidden rounded-full bg-white/15"><div className={`h-full ${moodDisplay.barClass}`} style={{ width: `${moodScore}%` }} /></div></div>
                  </div>
                </div>
              </div>
              <button type="button" onClick={onOpenPetRoom} className="group mt-3 flex w-full items-center gap-4 overflow-hidden rounded-3xl border border-white/10 bg-white/10 p-3 text-left transition hover:bg-white/15" title="Открыть комнату питомца">
                ...
                <span aria-hidden="true" className="rounded-full bg-white/15 px-3 py-2 text-xl font-black text-white/80">›</span>
              </button>
            </>
          )}
        </aside>
