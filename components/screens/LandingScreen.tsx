...
export const LandingScreen: React.FC<LandingScreenProps> = ({
  userProfile, isAuthenticated, hasActiveClassicGame = false, onStartClassic, onStartAnagrams,
  onStartSprint, onStartHangman, onStartMemory, onOpenShop, onOpenLogin, onOpenProfile, onOpenPetRoom,
}) => {
          ...
          {!isAuthenticated ? <div className="rounded-3xl border border-white/10 bg-white/10 p-5"><div className="mb-3 text-5xl">{getPetEmoji(userProfile.pet)}</div><h2 className="mb-2 text-xl font-black">Сохраните своего питомца</h2><p className="mb-4 text-sm leading-relaxed text-white/75">После входа будут сохраняться ₽, покупки, опыт и личный словарь.</p><button type="button" onClick={onOpenLogin} className="w-full rounded-2xl bg-white py-3 font-black text-indigo-700 transition hover:bg-indigo-50">Создать аккаунт</button></div> : <>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex min-h-[8.25rem] flex-col justify-between rounded-2xl border border-white/10 bg-white/10 p-3">
                ...
                <div><div className="mb-1 flex justify-between text-[10px] font-black uppercase tracking-widest text-white/65"><span>Опыт</span><span>{userProfile.pet.xp}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full bg-white" style={{ width: `${xpProgress}%` }} /></div></div></div>
                <div><div className="mb-1 flex justify-between text-[10px] font-black uppercase tracking-widest text-white/65"><span>Радость</span><span>{moodScore}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full bg-white/80" style={{ width: `${moodScore}%` }} /></div></div></div>
              </div>
            </div>
