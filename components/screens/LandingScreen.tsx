...
const GameIconButton: React.FC<GameOption> = ({ title, iconSrc, onStart, badge }) => (
  ...
  >
    {badge && (
      <span aria-hidden="true" className="absolute -right-1 -top-2 rounded-full bg-green-50 px-1.5 py-0.5 text-[8px] font-black uppercase text-green-700 sm:right-1 sm:top-1 sm:text-[9px]">
        {badge}
      </span>
    )}
    ...
    <span aria-hidden="true" className="mt-1 block max-w-full truncate text-[11px] font-black text-indigo-950 sm:mt-2 sm:text-sm lg:text-base">
      {title}
    </span>
  </button>
);
...
export const LandingScreen: React.FC<LandingScreenProps> = ({
  ...
  const gameOptions: GameOption[] = [
    {
      title: hasActiveClassicGame ? 'Продолжить' : 'Классика',
      iconSrc: '/assets/games/game_classic.webp',
      onStart: onStartClassic,
      badge: hasActiveClassicGame ? 'Сохранено' : undefined,
    },
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={onOpenShop} className="flex min-h-[8.25rem] flex-col justify-between rounded-2xl border border-white/10 bg-white/10 p-4 text-left transition hover:bg-white/15">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-bold uppercase tracking-widest text-white/65">рублей</div>
                  </div>
                </button>
                <div className="flex min-h-[8.25rem] flex-col justify-between rounded-2xl border border-white/10 bg-white/10 p-3">
                  <div className="flex items-end justify-between">
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/65">Уровень</span>
                    <span className="text-xl font-black">{userProfile.pet.level}</span>
                  </div>
                  <div>
