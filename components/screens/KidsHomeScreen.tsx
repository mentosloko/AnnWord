...
export const KidsHomeScreen: React.FC<Props> = ({ userProfile, dailyQuest, onStartDailyQuest, hasActiveClassicGame, hasActiveAnagramGame, savedGameType, onContinueSavedGame, activeDictionaryName, onStartClassic, onStartAnagrams, onStartTranslation, onStartSprint, onStartHangman, onStartMemory, onStartLetterSquare, onOpenShop, onOpenProfile, onOpenPetRoom, onOpenAdultRoom, onOpenDictionary, onOpenPremium }) => {
  ...
  const games: Game[] = [
    ...
    { title: '1 из 2', image: '/assets/games/game_one_of_two.webp', note: 'Выберите правильный перевод.', action: onStartTranslation, badge: savedBadge('translation') },
  ...
  return <ScreenContainer className="max-w-6xl pb-24 pt-4 sm:pb-20 sm:pt-6">
    <section className="grid min-h-[24rem] overflow-hidden rounded-[2rem] bg-gradient-to-br from-purple-700 to-indigo-700 text-white shadow-xl sm:min-h-0 lg:grid-cols-[1fr_21rem]">
      <div className="p-5 sm:p-8"><div className="text-xs font-bold uppercase tracking-wider text-white/65">Задание на сегодня</div><h1 className="mt-2 min-h-[4.75rem] text-3xl font-bold leading-tight sm:min-h-0 sm:text-5xl">{dailyQuest?.completed ? `Серия: ${streak} ${dayWord(streak)}` : dailyQuest?.title || 'Поиграем со словами?'}</h1><p className="mt-3 min-h-[4.5rem] max-w-2xl text-sm font-medium leading-relaxed text-white/80 sm:min-h-0 sm:text-base">{dailyQuest?.completed ? 'Задание выполнено. Питомец получил награду, а теперь можно выбрать любую игру.' : dailyQuest?.description || 'Короткая игра поможет продлить серию и порадовать питомца.'}</p>{onOpenDictionary ? <button type="button" onClick={onOpenDictionary} aria-label={`Выбрать слова для игр. Сейчас: ${dictionaryLabel}`} className={dictionaryPillClass}>{dictionaryPillContent}</button> : <div className={dictionaryPillClass}>{dictionaryPillContent}</div>}<div className="mt-6 min-h-[3.5rem] flex flex-col gap-2 sm:min-h-0 sm:flex-row"><button type="button" onClick={dailyQuest?.completed ? (continueAction || playRandomGame) : startQuest} className="rounded-2xl bg-white px-6 py-3.5 font-bold text-indigo-700 shadow-sm">{dailyQuest?.completed ? continueAction ? 'Продолжить игру' : 'Играть ещё' : 'Начать задание'}</button>{!dailyQuest?.completed && continueAction && <button type="button" onClick={continueAction} className="rounded-2xl bg-white/10 px-6 py-3.5 font-bold text-white ring-1 ring-white/25">Продолжить сохранённую</button>}</div></div>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
      ...
      <aside className="space-y-4"><SectionCard><div className="flex items-center justify-between gap-3"><div><div className={experienceUi.eyebrow}>Питомец</div><h2 className="mt-1 text-xl font-bold text-indigo-950">{userProfile.pet.name}</h2></div><button type="button" onClick={onOpenPetRoom} className="rounded-xl bg-purple-50 px-3 py-2 text-sm font-bold text-purple-700">Комната</button></div><div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-2xl bg-yellow-50 p-3"><CoinIcon className="text-xl" /><div className="mt-1 text-2xl font-bold text-yellow-700">{userProfile.coins}</div><div className="text-xs font-medium text-yellow-700/70">монет</div></div><div className="rounded-2xl bg-purple-50 p-3"><div className="text-sm font-bold text-purple-700">{mood.label}</div><div className="mt-3 h-2 overflow-hidden rounded-full bg-purple-100"><div className={`h-full ${mood.barClass}`} style={{ width: `${normalizeMoodScore(userProfile.pet)}%` }} /></div><div className="mt-2 text-xs font-medium text-purple-600">ур. {xp.level} · XP {xp.currentLevelXp}/{xp.xpForNextLevel}</div></div></div></SectionCard>
        <SectionCard><div className={experienceUi.eyebrow}>Быстрые действия</div><div className="mt-3 grid gap-2"><button type="button" onClick={onOpenShop} className={experienceUi.secondaryButton}>Магазин</button>{onOpenDictionary && <button type="button" onClick={onOpenDictionary} className={experienceUi.secondaryButton}>Выбрать словарь</button>}<button type="button" onClick={onOpenProfile} className={experienceUi.secondaryButton}>Твой прогресс</button><button type="button" onClick={onOpenAdultRoom} className={experienceUi.secondaryButton}>Кабинет родителя</button></div></SectionCard>
        {!hasPremium && onOpenPremium && <SectionCard className="border-amber-100 bg-amber-50"><div className="text-xs font-bold uppercase tracking-wider text-amber-600">Подписка</div><h2 className="mt-2 text-lg font-bold text-indigo-950">Kids Premium</h2><p className="mt-1 text-sm font-medium leading-relaxed text-slate-600">Школьные слова, свои подборки, отчёты и подключение преподавателя.</p><button type="button" onClick={onOpenPremium} className="mt-4 w-full rounded-2xl bg-amber-500 px-4 py-3 font-bold text-white shadow-sm transition hover:bg-amber-600">Подробнее о Kids Premium</button><p className="mt-2 text-xs font-medium leading-relaxed text-amber-800/70">Описание доступно без PIN. Перед переходом к оплате приложение попросит PIN родителя.</p></SectionCard>}
      </aside>
    </div>
  </ScreenContainer>;
};
