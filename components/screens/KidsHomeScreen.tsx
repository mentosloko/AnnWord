...
import { ReliablePetImage } from '../ReliablePetImage';
...
export const KidsHomeScreen: React.FC<Props> = ({ userProfile, dailyQuest, onStartDailyQuest, hasActiveClassicGame, hasActiveAnagramGame, onContinueSavedGame, activeDictionaryName, onStartClassic, onStartAnagrams, onStartTranslation, onStartSprint, onStartHangman, onStartMemory, onStartLetterSquare, onOpenShop, onOpenProfile, onOpenPetRoom, onOpenAdultRoom, onOpenDictionary, onOpenPremium }) => {
  ...
  return <ScreenContainer className="max-w-6xl pb-24 pt-4 sm:pb-20 sm:pt-6">
    <section className="grid overflow-hidden rounded-[2rem] bg-gradient-to-br from-purple-700 to-indigo-700 text-white shadow-xl lg:grid-cols-[1fr_21rem]">
      ...
      <button type="button" onClick={onOpenPetRoom} className="relative hidden min-h-[17rem] items-end justify-center overflow-hidden bg-white/10 lg:flex"><span className="sr-only">Открыть комнату питомца</span><ReliablePetImage key={petUrl || userProfile.pet.type || 'pet'} src={petUrl} fallbackEmoji={getPetEmoji(userProfile.pet)} className="h-72 w-72 object-contain object-center" fallbackClassName="pb-10 text-8xl" /></button>
    </section>
