...
export const PracticeHomeScreenWithLetterSquare: React.FC<Props> = ({ userProfile, dailyQuest, hasActiveClassicGame, hasActiveAnagramGame, activeDictionaryName, activeDictionaryWordCount, onStartDailyQuest, onStartClassic, onStartAnagrams, onStartTranslation, onStartSprint, onStartHangman, onStartMemory, onStartLetterSquare, onOpenDictionaryStudio }) => {
  ...
  return <ScreenContainer className="max-w-7xl pb-20 pt-5"><section className="rounded-[2rem] border-2 border-indigo-50 bg-white p-5 shadow-sm sm:p-6"><div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="text-xs font-black uppercase tracking-widest text-indigo-400">AnnWord Practice</div><div className="mt-2 flex items-start justify-between gap-3"><h1 className="text-3xl font-black leading-tight text-indigo-950 sm:text-5xl">{questCompleted ? 'Практика на сегодня выполнена' : 'Ежедневная практика'}</h1>{questCompleted && <span title="Выполнено" aria-label={`Выполнено. ${daysInRow} дней подряд`} className="inline-flex h-12 min-w-12 shrink-0 flex-col items-center justify-center rounded-2xl bg-green-100 px-3 text-green-700 shadow-sm"><span className="text-lg font-black leading-none">✓</span><span className="mt-0.5 text-xs font-black leading-none">{daysInRow}</span></span>}</div><p className="mt-3 max-w-2xl text-sm font-bold leading-relaxed text-gray-600 sm:text-base">{questCompleted ? 'Можно сыграть ещё в любую игру или вернуться завтра.' : dailyQuest?.description || 'Сыграйте одну короткую тренировку, чтобы продлить дни подряд.'}</p><div className="mt-5 flex flex-col gap-3 sm:flex-row"><button type="button" onClick={questCompleted ? scrollToGames : runDailyQuest} className="rounded-2xl bg-indigo-600 px-6 py-4 font-black text-white shadow-sm transition hover:bg-indigo-700">{questCompleted ? 'Выбрать игру' : 'Начать ежедневную тренировку'}</button><button type="button" onClick={onOpenDictionaryStudio} className="rounded-2xl border-2 border-indigo-100 bg-white px-4 py-3 text-sm font-black text-indigo-700">Словарь</button></div></div></div></section><section id="practice-games-section" className="mt-6 grid gap-6 xl:grid-cols-[1fr_24rem]"><div className="rounded-[2rem] border-2 border-indigo-50 bg-white p-5 shadow-sm sm:p-6"><div className="mb-4"><div className="text-xs font-black uppercase tracking-widest text-indigo-400">Игры</div><h2 className="mt-1 text-2xl font-black text-indigo-950">Выберите режим</h2></div><div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3">{cards.map(card => <CardTile key={card.title} {...card} />)}</div></div><aside className="space-y-4"><section className="rounded-[2rem] border-2 border-indigo-50 bg-white p-5 shadow-sm"><div className="text-xs font-black uppercase tracking-widest text-indigo-400">Активный словарь</div><h2 className="mt-2 text-2xl font-black text-indigo-950">{dictionaryName}</h2><p className="mt-2 text-sm font-bold text-gray-500">{dictionaryWordCount} слов доступно</p><button type="button" onClick={onOpenDictionaryStudio} className="mt-4 w-full rounded-2xl bg-indigo-600 px-5 py-3 font-black text-white">Сменить словарь</button></section></aside></section></ScreenContainer>;
};