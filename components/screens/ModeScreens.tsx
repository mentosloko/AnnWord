...
export const AnagramsScreen: React.FC<ModeScreenProps> = ({ words, dictionaryLabel, dictionaryIcon, userProfile, onGameReward, onWordPractice, onBackHome, onDictionaryPeek }) => { const profile = buildModeProfile(userProfile, words); return <GameModeShell title="Анаграммы" subtitle="Собери слово из букв" rules={rulesFor('anagrams', profile)} dictionaryWords={words} dictionaryLabel={dictionaryLabel} dictionaryIcon={dictionaryIcon} onBackHome={onBackHome} onDictionaryPeek={dictionaryPeekFor(profile, onDictionaryPeek)}><AnagramGame userProfile={profile} onGameReward={onGameReward} onWordPractice={onWordPractice} onBack={onBackHome} /></GameModeShell>; };
...
export const TranslationChoiceScreen: React.FC<ModeScreenProps> = ({ words, dictionaryLabel, dictionaryIcon, userProfile, onGameReward, onWordPractice, onBackHome, onDictionaryPeek }) => { const profile = buildModeProfile(userProfile, words); return <GameModeShell title="1 из 2" subtitle="Выбор перевода" rules={rulesFor('translation', profile)} dictionaryWords={words} dictionaryLabel={dictionaryLabel} dictionaryIcon={dictionaryIcon} onBackHome={onBackHome} onDictionaryPeek={dictionaryPeekFor(profile, onDictionaryPeek)}><TranslationChoiceGame userProfile={profile} onGameReward={onGameReward} onWordPractice={onWordPractice} onBack={onBackHome} /></GameModeShell>; };
