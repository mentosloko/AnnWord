      ...
      <main className="-mx-4 overflow-hidden bg-white sm:mx-0">
        ...
        <section className="px-4 pb-8 sm:px-8 sm:pb-10 lg:px-10">
          <div className="overflow-hidden rounded-[2.35rem] bg-gradient-to-r from-indigo-700 via-violet-600 to-sky-500 p-5 text-white shadow-2xl shadow-indigo-700/20 sm:p-7">
            <div className="grid gap-6 lg:grid-cols-[minmax(25rem,0.9fr)_minmax(0,1.1fr)] lg:items-center">
              <div className="flex items-center gap-4 sm:gap-5"><DeferredImage src={asset('cta-mascot.webp')} mobileSrc={asset('cta-mascot-mobile.webp')} rootMargin="96px 0px" alt="Питомец AnnWord летит за наградами" loading="lazy" decoding="async" className="h-20 w-20 shrink-0 object-contain object-center drop-shadow-2xl sm:h-28 sm:w-28 lg:h-32 lg:w-32" /><div className="max-w-[18rem]"><h2 className="text-xl font-black leading-tight sm:text-3xl">Играй и получай награды!</h2><p className="mt-1.5 text-xs font-bold leading-relaxed text-indigo-100 sm:mt-2 sm:text-sm">Монеты, кристаллы и вещи для питомца превращают усилия в видимый результат.</p></div></div>
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-6 sm:gap-3">{rewards.map(([src, label]) => <div key={label} className="flex min-h-[8.5rem] flex-col justify-center rounded-2xl bg-white/95 p-2 text-center text-indigo-950 shadow-lg sm:min-h-[9.5rem]"><DeferredImage src={src} mobileSrc={mobileVariant(src)} rootMargin="96px 0px" alt={label} loading="lazy" decoding="async" className="mx-auto h-14 w-14 object-contain sm:h-16 sm:w-16" /><div className="mt-1 text-[10px] font-black leading-tight sm:text-xs">{label}</div></div>)}</div>
            </div>
          </div>
        </section>
