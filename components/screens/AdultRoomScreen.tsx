...
export const AdultRoomScreen: React.FC<Props> = ({ userProfile, onBackHome, onOpenDictionaryStudio }) => {
  ...
  const trialPremium = premiumActive && Boolean(userProfile.kidsTrialStartedAt && userProfile.kidsTrialExpiresAt);
  ...
  if (isParent && !isTeacher && !unlocked) return <ScreenContainer className="max-w-md pb-20">
    ...
    <section className="rounded-[2rem] bg-white p-6 shadow-sm">
      ...
      <div id="parent-pin-error" className="mt-4"><StableStatusSlot message={pinError} tone="error" role="alert" /></div>
      ...
      <button type="submit" disabled={!pinReady || busyAction === 'unlock' || busyAction === 'pin-reset'} className="mt-4 w-full rounded-xl bg-indigo-600 p-3 font-black text-white disabled:bg-gray-200 disabled:text-gray-400">{busyAction === 'unlock' ? 'Открываю кабинет...' : 'Открыть'}</button>
  ...
  return <ScreenContainer className="max-w-6xl pb-20">
    ...
    {!isTeacher && (premiumChecking ? <section className="mb-5 rounded-3xl border-2 border-indigo-100 bg-indigo-50 p-4"><div className="text-xs font-black uppercase tracking-widest text-indigo-500">Проверяем доступ</div><p className="mt-1 text-sm font-bold text-gray-600">Обновляем данные подписки с сервера…</p></section> : <section className={`mb-5 rounded-3xl border-2 p-4 ${premiumActive ? 'border-green-100 bg-green-50' : 'border-amber-100 bg-amber-50'}`}><div className={`text-xs font-black uppercase tracking-widest ${premiumActive ? 'text-green-600' : 'text-amber-600'}`}>{premiumActive ? trialPremium ? 'Пробный Kids Premium активен' : 'Kids Premium активен' : 'Бесплатный Kids'}</div><p className="mt-1 text-sm font-bold text-gray-600">{premiumActive ? `Доступ открыт ${formatPremiumAccessPeriod(userProfile.premiumExpiresAt)}.` : 'Игры, питомец, магазин и общий детский словарь доступны бесплатно. Код преподавателя, отчёты и расширенные словари открываются в Premium.'}</p></section>)}
    ...
    <div className="grid min-w-0 gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      {learner ? <main className="min-w-0 space-y-4">
        ...
        <section className="min-w-0 rounded-3xl bg-white p-4 shadow-sm sm:p-5"><h2 className="font-black">Частые слова</h2>{encounteredWords.length ? <div className="mt-3 grid min-w-0 gap-2 sm:grid-cols-2">{encounteredWords.slice(0, 12).map(word => <div key={word.word} className="min-w-0 rounded-2xl bg-indigo-50 p-3"><div className="break-all font-black text-indigo-950">{word.word}</div><div className="break-words text-xs font-bold text-indigo-500">{attempts(word)} {russianPlural(attempts(word), ['попытка', 'попытки', 'попыток'])} · точность: {accuracy(word)}% · {word.mistakes} {russianPlural(word.mistakes, ['ошибка', 'ошибки', 'ошибок'])}</div></div>)}</div> : <EmptyCard>После игр здесь появится статистика слов.</EmptyCard>}</section>
