...
export const AdultRoomScreen: React.FC<Props> = ({ userProfile, onBackHome, onOpenDictionaryStudio }) => {
  ...
  if (isParent && !isTeacher && !unlocked) return <ScreenContainer className="max-w-md pb-20">
    ...
    <section className="rounded-[2rem] bg-white p-6 shadow-sm">
      ...
      <button type="submit" disabled={busyAction === 'unlock' || busyAction === 'pin-reset'} className="mt-4 w-full rounded-xl bg-indigo-600 p-3 font-black text-white disabled:bg-gray-200 disabled:text-gray-400">{busyAction === 'unlock' ? 'Открываю кабинет...' : 'Открыть'}</button>
