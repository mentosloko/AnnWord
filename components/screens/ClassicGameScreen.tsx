...
import { russianPlural } from '../../utils/textUtils';
...
export const ClassicGameScreen: React.FC<Props> = ({ gameState, settings, userProfile, isAuthenticated, rulesViewerKey = 'guest', keyStatuses, shakeRowIndex, dictionaryWords = [], onChar, onDelete, onEnter, onHint, onRestart, onBackHome, onRegister }) => {
  ...
  return (
    <ScreenContainer compact className="h-[100dvh] min-h-[100svh] max-w-none overflow-hidden px-1.5 py-1.5 sm:px-3 sm:py-2 lg:px-5">
      <div className="relative mx-auto grid h-full min-h-0 w-full max-w-[88rem] grid-rows-[auto_minmax(1.25rem,auto)_minmax(0,1fr)_auto] gap-[clamp(0.2rem,0.7dvh,0.55rem)] overflow-hidden">
        ...
        <div aria-live="polite" className="min-h-[1.25rem] text-center text-xs font-black leading-tight text-rose-500 sm:text-sm">{gameState.error || ''}</div>
        {showHint && <div role="dialog" aria-label="Подсказка" className="absolute left-1/2 top-11 z-30 w-[min(20rem,calc(100vw-1rem))] -translate-x-1/2 rounded-2xl border-2 border-blue-100 bg-white p-3 text-xs text-blue-950 shadow-2xl sm:rounded-3xl sm:p-4 sm:text-sm"><div className="flex gap-3"><div className="min-w-0 flex-1 break-words font-bold leading-snug">{gameState.loadingHint ? 'Готовлю подсказку...' : gameState.hint || 'Подсказка скоро появится.'}</div><button type="button" aria-label="Закрыть подсказку" onClick={() => { setShowHint(false); blur(); }} className="shrink-0 font-black text-blue-400">×</button></div></div>}
        ...
        <footer className="flex min-h-0 shrink-0 justify-center overflow-hidden"><Keyboard onChar={onChar} onDelete={onDelete} onEnter={onEnter} letterStatuses={keyStatuses} /></footer>
      </div>

      {showRules && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-indigo-950/55 p-3 backdrop-blur-sm" role="presentation"><div role="dialog" aria-modal="true" aria-labelledby="classic-rules-title" className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-y-auto overscroll-contain rounded-[2rem] border-2 border-indigo-100 bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-widest text-indigo-400">{seen ? 'Правила игры' : 'Первый запуск'}</div><h2 id="classic-rules-title" className="mt-1 text-2xl font-black text-indigo-950">{seen ? 'Правила игры «Классика»' : 'Как играть в «Классику»'}</h2></div><button type="button" aria-label="Закрыть правила" onClick={closeRules} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-xl font-black text-indigo-500">×</button></div><ul className="mt-5 space-y-3"><li className="flex gap-3 rounded-2xl bg-indigo-50/70 px-4 py-3 text-sm font-bold text-indigo-950"><span aria-hidden="true" className="text-indigo-500">✓</span><span>Угадайте английское слово за шесть попыток.</span></li><li className="flex gap-3 rounded-2xl bg-indigo-50/70 px-4 py-3 text-sm font-bold text-indigo-950"><span aria-hidden="true" className="text-indigo-500">✓</span><span>Зелёная буква стоит верно, жёлтая есть в слове, серая отсутствует.</span></li><li className="flex gap-3 rounded-2xl bg-indigo-50/70 px-4 py-3 text-sm font-bold text-indigo-950"><span aria-hidden="true" className="text-indigo-500">✓</span><span>Введите слово на клавиатуре и нажмите «Ввод».</span></li></ul><button type="button" onClick={closeRules} className="mt-5 w-full rounded-2xl bg-indigo-600 px-5 py-4 font-black text-white">{seen ? 'Закрыть правила' : 'Начать игру'}</button><p className="mt-3 text-center text-xs font-bold text-gray-400">Правила всегда можно открыть снова кнопкой «?».</p></div></div>}

      {finished && reward && <GameResultOverlay isOpen status={gameState.gameStatus === 'won' ? 'won' : 'lost'} title={gameState.gameStatus === 'won' ? 'Победа!' : 'Почти получилось'} subtitle={authenticated ? (gameState.gameStatus === 'won' ? 'Слово угадано.' : 'Попробуем ещё раз?') : 'Создайте аккаунт в Kids или Practice, чтобы сохранять прогресс и открыть словари.'} emoji={gameState.gameStatus === 'won' ? '🎉' : '💪'} pet={progress?.pet} xpGained={showKidsRewards ? reward.xp : 0} coinsGained={showKidsRewards ? reward.coins : 0} primaryLabel={authenticated ? 'Играть снова' : 'Создать аккаунт'} secondaryLabel={authenticated ? 'В меню' : 'На главную'} onPrimary={authenticated ? restart : register} onSecondary={onBackHome} scoreboard={authenticated ? <PersonalScoreboard gameId="classic" userKey={userProfile.username} value={Math.max(1, gameState.guesses.length)} direction="lower" unit="попыток" record={gameState.gameStatus === 'won'} /> : undefined} details={<span>Слово: <b>{gameState.secretWord}</b>{gameState.secretWordData?.translation ? ` · ${gameState.secretWordData.translation}` : ''}{spent && showKidsRewards ? ` · подсказка: −${spent} ${russianPlural(spent, ['монета', 'монеты', 'монет'])}` : ''}</span>} />}
    </ScreenContainer>
  );
};
