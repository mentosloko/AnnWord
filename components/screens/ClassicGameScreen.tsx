      ...
      <div className="relative mx-auto grid h-full min-h-0 w-full max-w-[88rem] grid-rows-[auto_auto_minmax(0,1fr)_auto] gap-[clamp(0.2rem,0.7dvh,0.55rem)] overflow-hidden">
        <header className="grid shrink-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-1.5 sm:gap-2">
          ...
          <div className="flex items-center gap-1">
            <DictionaryPeek words={userProfile.customDictionaryEn} wordLength={settings.wordLength} iconOnly locked={!authenticated} lockedMessage="Мой словарь доступен после регистрации в Kids или Practice." onBeforeOpen={showKidsRewards ? onDictionaryPeek : undefined} chargeLabel="Просмотр словаря стоит как подсказка." />
        ...
        {showRules && <div role="dialog" aria-label="Как играть" className="absolute right-0 top-11 z-30 w-[min(21rem,92vw)] rounded-3xl border-2 border-indigo-100 bg-white p-4 text-sm text-indigo-950 shadow-2xl"><div className="mb-3 flex justify-between font-black"><span>Как играть</span><button type="button" aria-label="Закрыть правила" onClick={closeRules}>×</button></div><p className="font-bold">Соберите слово за 6 попыток. Зелёная буква стоит верно, жёлтая есть в слове, серая отсутствует.</p></div>}
