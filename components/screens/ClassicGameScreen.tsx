...
const WORDLE_RULES_SEEN_KEY = 'annword:wordle-rules-seen';
...
export const ClassicGameScreen: React.FC<ClassicGameScreenProps> = ({
  ...
  useEffect(() => {
    ...
    const seen = window.localStorage.getItem(WORDLE_RULES_SEEN_KEY) === 'true';
    setHasSeenRules(seen);
  ...
  const closeRules = () => {
    ...
    if (typeof window !== 'undefined') window.localStorage.setItem(WORDLE_RULES_SEEN_KEY, 'true');
    blurActiveElement();
  };
      ...
      <div className="relative mx-auto grid h-full min-h-0 w-full max-w-[88rem] grid-rows-[auto_minmax(0,1fr)_auto] gap-2 overflow-hidden lg:gap-3">
        <header className="grid shrink-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button type="button" onClick={handleRulesClick} aria-label="Правила Wordle" title="Правила" className={`flex h-9 w-9 items-center justify-center rounded-xl border font-black transition sm:h-10 sm:w-10 ${hasSeenRules ? 'border-indigo-50 bg-white text-indigo-300 hover:bg-indigo-50 hover:text-indigo-500' : 'border-indigo-100 bg-indigo-50 text-indigo-700 hover:bg-indigo-100'}`}>?</button>
        ...
        {showRules && (
          <div className="absolute right-0 top-12 z-30 w-[min(21rem,92vw)] rounded-3xl border-2 border-indigo-100 bg-white p-4 text-sm text-indigo-950 shadow-2xl sm:top-14">
            <div className="mb-3 flex items-center justify-between gap-3"><div className="font-black">Как играть</div><button type="button" onClick={closeRules} className="font-black text-indigo-400">×</button></div>
        ...
        <section className="flex min-h-0 items-center justify-center overflow-hidden rounded-[1.5rem] border-2 border-indigo-50 bg-white/85 px-2 py-2 shadow-sm sm:rounded-[2rem] sm:px-4 lg:border-0 lg:bg-transparent lg:px-0 lg:shadow-none">
