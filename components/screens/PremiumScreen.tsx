...
export const PremiumScreen: React.FC<PremiumScreenProps> = ({ userProfile, onBack, onOpenDictionarySetup, onTestUnlockPremium }) => {
  ...
  const paymentPlanButtons = <div className="grid gap-3 sm:grid-cols-2">
      ...
      <section className="overflow-hidden rounded-[2.25rem] border-2 border-amber-100 bg-white shadow-sm">
        <div className="grid gap-6 p-6 lg:grid-cols-[1.05fr_0.95fr] lg:p-8">
          <div>
            ...
            <div className="mt-6">
              {hasPremium ? <div className="space-y-4">
                <button type="button" onClick={onOpenDictionarySetup} className="rounded-2xl bg-indigo-600 px-6 py-4 font-black text-white shadow-sm transition hover:bg-indigo-700">{kidsMode ? 'Выбрать слова для ребёнка' : 'Выбрать слова для тренировки'}</button>
                {PAYMENTS_ENABLED && <div className="rounded-3xl border-2 border-amber-100 bg-amber-50/50 p-4"><div className="mb-3"><div className="text-sm font-black text-amber-900">Продлить Premium</div><p className="mt-1 text-xs font-bold leading-relaxed text-amber-700">Новый оплаченный период добавится к уже действующему сроку. Это также позволяет повторно проверить тестовую оплату.</p></div>{paymentPlanButtons}</div>}
              </div> : accessChecking ? <div role="status" aria-live="polite" className="max-w-xl rounded-2xl border-2 border-indigo-100 bg-indigo-50 px-5 py-4"><div className="font-black text-indigo-800">Проверяем ваш Premium-доступ…</div><p className="mt-1 text-sm font-bold text-indigo-600">Кнопки оплаты появятся только после подтверждения текущего тарифа сервером.</p></div> : PAYMENTS_ENABLED ? paymentPlanButtons : DEV_TRIAL_ENABLED && onTestUnlockPremium ? <button type="button" onClick={onTestUnlockPremium} className="rounded-2xl bg-amber-500 px-6 py-4 font-black text-white shadow-sm transition hover:bg-amber-600">Открыть Premium на 7 дней</button> : <button type="button" disabled className="rounded-2xl bg-gray-100 px-6 py-4 font-black text-gray-400">Оплата скоро будет подключена</button>}
              {!accessChecking && PAYMENTS_ENABLED && <p className="mt-3 max-w-2xl text-xs font-bold leading-relaxed text-gray-500">{kidsMode && 'Перед переходом к оплате понадобится PIN родителя. '}Переходя к оплате, вы подтверждаете, что ознакомились и согласны с условиями <a href={LEGAL_DOCUMENTS.publicOffer} {...LEGAL_LINK_PROPS} className="font-black text-indigo-700 underline decoration-indigo-200 underline-offset-2 transition hover:text-indigo-900 hover:decoration-indigo-500">Публичной оферты</a>.</p>}
