    ...
    <section className="overflow-hidden rounded-[2.25rem] border-2 border-amber-100 bg-white shadow-sm">
      <div className="grid gap-6 p-6 lg:grid-cols-[1.05fr_0.95fr] lg:p-8">
        <div>
          ...
          <h1 className="mt-5 text-4xl font-black leading-tight text-indigo-950 sm:text-5xl">10 тематических словарей с уровнями сложности</h1>
          <p className="mt-4 max-w-2xl text-base font-bold leading-relaxed text-gray-600">Откройте {totalWords} игровых слов: Business, Travel, Medicine, IELTS, IT, Finance, Legal, Science, Everyday+ и Food. В словарях есть уровни A1–C2, а каждый набор сейчас содержит от {minWords} до {maxWords} слов.</p>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            {hasPremium ? <button type="button" onClick={onOpenDictionarySetup} className="rounded-2xl bg-indigo-600 px-6 py-4 font-black text-white shadow-sm transition hover:bg-indigo-700">Выбрать Premium-словарь</button> : PAYMENTS_ENABLED && onTestUnlockPremium ? <button type="button" onClick={onTestUnlockPremium} className="rounded-2xl bg-amber-500 px-6 py-4 font-black text-white shadow-sm transition hover:bg-amber-600">Тестово открыть Premium</button> : <button type="button" disabled className="rounded-2xl bg-gray-100 px-6 py-4 font-black text-gray-400">Оплата скоро будет подключена</button>}
          ...
          {!hasPremium && <p className="mt-3 text-xs font-bold leading-relaxed text-gray-400">В production тестовое открытие Premium отключено. Доступ будет выдаваться после оплаты и webhook-подтверждения.</p>}
        </div>
