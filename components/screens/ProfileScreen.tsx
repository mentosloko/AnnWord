  ...
  return <ScreenContainer className="max-w-5xl pb-24 pt-4 sm:pb-20 sm:pt-6">
    ...
    {tab === 'overview' && <div className="mt-5 space-y-5">
      {isTeacher ? <>
        ...
        <SectionCard><div className={experienceUi.eyebrow}>Рабочие материалы</div><div className="mt-4 grid grid-cols-2 gap-3"><MetricCard value={(userProfile.dictionaryCollections || []).length} label="словарей" /><MetricCard value={teacherSummary.learnedWords} label="выучено учениками" /></div><p className="mt-3 text-sm font-medium text-slate-500">Сводка строится по тому же набору подключённых учеников, который используется в разделе «Ученики».</p></SectionCard>
      </> : <>
    ...
    {tab === 'subscription' && !isTeacher && <div className="mt-5 space-y-5"><SectionCard><div className="flex items-start justify-between gap-3"><div><div className={experienceUi.eyebrow}>Premium</div><h2 className="mt-1 text-2xl font-bold text-indigo-950">{premium ? trialPremium ? 'Пробный Premium подключён' : 'Доступ подключён' : 'Бесплатный план'}</h2><p className="mt-2 text-sm font-medium text-slate-500">{premium ? `Действует ${premiumPeriod}.` : 'Базовые игры и встроенный словарь доступны бесплатно.'}</p></div>{premium && <span className="rounded-full bg-amber-50 px-3 py-1 text-sm font-bold text-amber-700">✦ активен</span>}</div></SectionCard>
