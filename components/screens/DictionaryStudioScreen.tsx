...
export const DictionaryStudioScreen: React.FC<DictionaryStudioScreenProps> = ({ userProfile, onBack, onSaveDictionary }) => {
  const canCreate = userProfile.subscriptionTier === 'premium' || ['admin', 'parent', 'teacher'].includes(userProfile.role || '');
  ...
  return (
    <ScreenContainer className="max-w-6xl pb-20">
      ...
      {!canCreate && <div className="mb-5 rounded-3xl border-2 border-purple-100 bg-purple-50 p-5"><h2 className="text-xl font-black text-purple-950">Собственные словари — в Premium</h2><p className="mt-2 text-sm font-bold text-purple-700">Сканируйте списки слов, собирайте темы и назначайте их ребёнку или классу.</p></div>}
