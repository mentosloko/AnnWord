  ...
  const save = async () => {
    ...
    try {
      await onSaveDictionary({
        title: title.trim() || 'Новый словарь',
        words,
        source,
  ...
  return (
    <ScreenContainer className="max-w-6xl pb-20">
      <header className="mb-5 flex items-center justify-between gap-3">
        ...
        <div className="text-center"><div className="text-xs font-black uppercase tracking-widest text-purple-500">{isTeacher ? 'Преподаватель' : 'Premium'}</div><h1 className="text-2xl font-black text-indigo-950 sm:text-3xl">Студия словарей</h1></div>
      ...
      {!canCreate && <div className="mb-5 rounded-3xl border-2 border-purple-100 bg-purple-50 p-5"><h2 className="text-xl font-black text-purple-950">Собственные словари — в Premium</h2><p className="mt-2 text-sm font-bold text-purple-700">Premium выдаётся вручную на месяц. После активации здесь появятся создание, загрузка и OCR словарей.</p></div>}
      {isTeacher && <div className="mb-5 rounded-3xl border-2 border-indigo-100 bg-indigo-50 p-5"><h2 className="text-xl font-black text-indigo-950">Словари преподавателя</h2><p className="mt-2 text-sm font-bold text-indigo-700">Создайте подборку вручную, затем назначьте её подключённому ученику. Распознавание рукописного текста появится позднее.</p></div>}
