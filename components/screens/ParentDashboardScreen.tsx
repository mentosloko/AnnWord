...
export const ParentDashboardScreen: React.FC<Props> = ({ userProfile, onBackHome, onOpenDictionaryStudio }) => {
  ...
  return <ScreenContainer className="max-w-6xl pb-24 pt-4 sm:pb-20 sm:pt-6">
    ...
    {learner && tab === 'words' && <div className="mt-5 grid gap-5 lg:grid-cols-2"><SectionCard><div className="text-xs font-bold uppercase tracking-wider text-rose-500">Нужно закрепить</div><h2 className="mt-1 text-xl font-bold text-indigo-950">Слова для повторения</h2><div className="mt-4 flex flex-wrap gap-2">{learning.activeReview.length ? learning.activeReview.slice(0, 40).map(item => <span key={item.word} className="rounded-full bg-rose-50 px-3 py-2 text-sm font-bold text-rose-700">{item.word}</span>) : <ExperienceState compact kind="success" title="Активных ошибок нет" />}</div></SectionCard><SectionCard><div className="text-xs font-bold uppercase tracking-wider text-emerald-600">Уже получается</div><h2 className="mt-1 text-xl font-bold text-indigo-950">Недавно исправлено</h2><div className="mt-4 flex flex-wrap gap-2">{fixed.length ? fixed.map(item => <span key={item.word} className="rounded-full bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700">{item.word}</span>) : <ExperienceState compact title="Пока нет исправленных слов" />}</div></SectionCard><SectionCard className="lg:col-span-2"><div className={experienceUi.eyebrow}>Чаще всего встречались</div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{encountered.length ? encountered.slice(0, 12).map(word => <div key={word.word} className="rounded-2xl bg-indigo-50/70 p-3"><div className="font-bold text-indigo-950">{word.word}</div><div className="mt-1 text-xs font-medium text-indigo-500">{attempts(word)} попыток · точность {accuracy(word)}%</div></div>) : <ExperienceState compact title="Статистика появится после игр" />}</div></SectionCard></div>}
