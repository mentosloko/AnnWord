...
const PremiumPackTile: React.FC<PremiumPack> = ({ title, words, description, icon }) => <button type="button" className="group relative rounded-2xl border-2 border-amber-100 bg-amber-50/70 p-3 text-left transition hover:-translate-y-0.5 hover:bg-amber-50 hover:shadow-md"><span className="absolute right-2 top-2 text-sm" aria-hidden="true">🔒</span><div className="flex items-start gap-2"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-white text-xl shadow-sm" aria-hidden="true">{icon}</span><div className="min-w-0"><div className="truncate pr-5 text-sm font-black text-amber-950">{title}</div><div className="text-xs font-black text-amber-700">{words} слов</div></div></div><p className="mt-2 min-h-[2rem] text-xs font-bold leading-snug text-amber-900/75">{description}</p><span className="mt-2 inline-flex rounded-full bg-white px-2 py-1 text-[10px] font-black uppercase tracking-widest text-amber-700 shadow-sm">{T.premiumBadge}</span></button>;
