    ...
    <section className="rounded-[2rem] border-2 border-indigo-50 bg-white p-5 shadow-sm sm:p-8">
      ...
      <form onSubmit={submit} className="mt-7 grid gap-5" noValidate>
        <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          <section className="rounded-[1.75rem] border-2 border-purple-50 bg-purple-50/60 p-4 sm:p-5"><div className="text-xs font-black uppercase tracking-widest text-purple-400">2. Родительский блок</div><div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="text-xs font-black uppercase tracking-widest text-amber-600">Первый месяц Kids Premium бесплатно</div><h2 className="mt-1 text-lg font-black text-indigo-950">Словари по классам и свои слова — сразу после настройки</h2><p className="mt-2 text-xs font-bold leading-5 text-slate-600">Пробный период уже привязан к аккаунту и закончится автоматически через месяц. После него бесплатные игры и общий детский словарь останутся доступны.</p></div><p className="mt-4 text-sm font-bold leading-relaxed text-gray-600">PIN нужен, чтобы ребёнок случайно не попал в настройки взрослого.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-black text-indigo-950">PIN родителя<input value={pin} onChange={event => handlePinChange(event.target.value, setPin)} type="password" inputMode="numeric" maxLength={4} autoComplete="new-password" placeholder="4 цифры" className="rounded-2xl border-2 border-indigo-100 bg-white px-4 py-3 text-base font-bold outline-none focus:border-indigo-400" /></label><label className="grid gap-2 text-sm font-black text-indigo-950">Повторите PIN<input value={pinRepeat} onChange={event => handlePinChange(event.target.value, setPinRepeat)} type="password" inputMode="numeric" maxLength={4} autoComplete="new-password" placeholder="4 цифры" className="rounded-2xl border-2 border-indigo-100 bg-white px-4 py-3 text-base font-bold outline-none focus:border-indigo-400" /></label></div><div className="mt-4 rounded-2xl bg-white px-4 py-3 text-xs font-bold leading-5 text-purple-900">На новом устройстве потребуется обычный вход в аккаунт. PIN не заменяет пароль.</div></section>
        </div>
        <div className="flex items-start gap-3 rounded-2xl border-2 border-indigo-100 bg-white px-4 py-4 text-sm font-bold leading-6 text-slate-700"><input id="child-data-consent" type="checkbox" checked={legalRepresentativeConsent} onChange={event => { setLegalRepresentativeConsent(event.target.checked); setError(null); }} className="mt-0.5 h-5 w-5 shrink-0 rounded" /><label htmlFor="child-data-consent">Я подтверждаю, что являюсь родителем или иным законным представителем ребёнка, и принимаю <a href={LEGAL_DOCUMENTS.childDataConsent} {...LEGAL_LINK_PROPS} className="font-black text-indigo-700 underline">Согласие на обработку персональных данных ребёнка</a>.</label></div>
        <StableStatusSlot message={error || pinHint} tone={error ? 'error' : 'warning'} role={error ? 'alert' : 'status'} />
        <button type="submit" disabled={isSaving || !legalRepresentativeConsent} className="rounded-2xl bg-indigo-600 px-5 py-4 text-base font-black text-white disabled:opacity-60">{isSaving ? 'Сохраняю…' : 'Продолжить к питомцу'}</button>
      </form>
    </section>
  </ScreenContainer>;
};
