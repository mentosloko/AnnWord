        ...
        {source === 'premium' && hasPremium && <section className="mt-4 rounded-3xl border-2 border-amber-100 bg-amber-50/55 p-4">
          ...
          {spotlightActive && <div className="mt-4 rounded-3xl border border-amber-200 bg-white p-4">
            ...
            {(spotlightSections.length > 0 || spotlightLoadState === 'ready') && <div className="mt-2 grid grid-cols-1 gap-1.5 sm:grid-cols-2 lg:grid-cols-3" role="group" aria-label="Модуль Spotlight">
              ...
              {spotlightSections.map(section => <button type="button" key={section.id} aria-label={`${section.title}, ${formatWordCount(section.wordCount)}`} aria-pressed={selectedSpotlightSectionId === section.id} onClick={() => selectSpotlightSection(section.id)} className={`flex min-h-12 items-center justify-between gap-2 rounded-xl border-2 px-3 py-2.5 text-left transition ${selectedSpotlightSectionId === section.id ? 'border-amber-400 bg-amber-100' : 'border-indigo-50 bg-white hover:border-indigo-200'}`}><span className="min-w-0 text-sm font-black leading-tight text-indigo-950">{section.title}</span><span className="shrink-0 rounded-full bg-indigo-50 px-2 py-1 text-[10px] font-black text-slate-500">{formatWordCount(section.wordCount)}</span></button>)}
            </div>}
          </div>}
        </section>}
