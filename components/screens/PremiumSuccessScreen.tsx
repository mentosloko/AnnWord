...
export const PremiumSuccessScreen: React.FC<PremiumSuccessScreenProps> = ({ userProfile, onPrimaryAction, onBackHome }) => {
  ...
  const statusText = useMemo(() => {
    ...
    if (confirmationState === 'delayed') return 'Подтверждение ещё идёт';
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      else if (!cancelled) setConfirmationState('delayed');
    };
    void poll();
    ...
    <section className="overflow-hidden rounded-[2.5rem] border-2 border-green-100 bg-white shadow-sm">
      <div className="grid gap-6 p-6 lg:grid-cols-[0.9fr_1.1fr] lg:p-8">
        <div className="rounded-[2rem] bg-gradient-to-br from-green-50 via-emerald-50 to-indigo-50 p-6">
          <div className="flex h-24 w-24 items-center justify-center rounded-[2rem] bg-white text-5xl shadow-sm" aria-hidden="true">{showNextStep ? '✅' : '⏳'}</div>
          ...
          <h1 className="mt-4 text-4xl font-black leading-tight text-indigo-950 sm:text-5xl">{showNextStep ? (kidsMode ? 'Kids Premium подключён' : 'AnnWord Premium подключён') : 'Подключаем Premium'}</h1>
          <p className="mt-4 text-base font-bold leading-relaxed text-gray-600">{showNextStep ? 'Доступ уже активирован. Теперь можно выбрать тему или добавить слова, которые нужно повторить в играх.' : confirmationState === 'delayed' ? 'Оплата принята, но подтверждение от платёжной системы может прийти с задержкой. Можно вернуться на главную — AnnWord обновит Premium автоматически после подтверждения.' : 'Оплата принята. AnnWord проверяет подтверждение платежа. Обычно это занимает несколько секунд, страницу обновлять не нужно.'}</p>
        ...
        <div className="flex flex-col justify-center">
          {showNextStep ? <>
            ...
            <p className="mt-3 text-sm font-bold leading-relaxed text-gray-600">Платёжная система уже вернула вас в AnnWord. Теперь приложение сверяет оплату с сервером и включит Premium автоматически.</p>
            <div className="mt-5 h-3 overflow-hidden rounded-full bg-white"><div className="h-full w-1/3 animate-[premiumLoading_1.2s_ease-in-out_infinite] rounded-full bg-indigo-600" /></div>
            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
              {confirmationState === 'delayed' && <button type="button" onClick={() => { setConfirmationState('checking'); void profileApiService.getCurrentProfile().then(profile => { if (profile.premiumExpiresAt) { setConfirmedProfile(profile); setConfirmationState('confirmed'); window.dispatchEvent(new CustomEvent('annword:profile-updated', { detail: profile })); } else setConfirmationState('delayed'); }).catch(() => setConfirmationState('delayed')); }} className="rounded-2xl bg-indigo-600 px-6 py-4 font-black text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-indigo-700">Проверить ещё раз</button>}
              <button type="button" onClick={onBackHome} className="rounded-2xl border-2 border-indigo-100 bg-white px-6 py-4 font-black text-indigo-700 transition hover:bg-indigo-50">На главную</button>
            </div>
