...
export const SetupScreen: React.FC<SetupScreenProps> = ({ selectedPlayMode, settings, customDictionaryWords, setupError, isUploadingDictionary, isAuthenticated, userProfile, onSettingsChange, onFileUpload: _onFileUpload, onOpenDictionaryStudio, onStartGame, onBack, onLogin }) => {
  const isCustomDictionary = settings.dictionarySource === 'custom';
  const premiumNotExpired = !userProfile.premiumExpiresAt || new Date(userProfile.premiumExpiresAt).getTime() > Date.now();
  ...
  return <ScreenContainer className="max-w-3xl px-3 pb-20 pt-3 sm:px-4">
    ...
    <div className="rounded-[2rem] border-2 border-indigo-50 bg-white p-4 shadow-sm sm:p-6">
      ...
      <button type="button" onClick={onStartGame} disabled={!canStart} className={`mt-5 w-full rounded-2xl py-4 font-black ${canStart ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{canStart ? `Начать: ${MODE_LABELS[selectedPlayMode]}` : !hasPremium && isCustomDictionary ? 'Нужен Premium' : `Нет слов из ${settings.wordLength} букв`}</button>
    </div>
  </ScreenContainer>;
};
