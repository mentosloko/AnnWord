...
export const SetupScreen: React.FC<SetupScreenProps> = ({ selectedPlayMode, settings, customDictionaryWords, setupError, isUploadingDictionary, isAuthenticated, userProfile, onSettingsChange, onFileUpload, onOpenDictionaryStudio, onStartGame, onBack, onLogin }) => {
  ...
  return <ScreenContainer className="max-w-3xl px-3 pb-20 pt-3 sm:px-4">
    ...
    <div className="rounded-[2rem] border-2 border-indigo-50 bg-white p-4 shadow-sm sm:p-6">
      ...
      {isCustomDictionary && hasPremium && <section className="mt-4 rounded-2xl border-2 border-dashed border-purple-100 bg-purple-50/50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center"><label className="flex flex-1 cursor-pointer items-center gap-3"><input type="file" accept=".txt,.csv" onChange={onFileUpload} className="hidden" /><span className="text-2xl">📄</span><span><span className="block font-black text-indigo-950">{customDictionaryWords.length ? 'Обновить список' : 'Загрузить TXT/CSV'}</span><span className="block text-xs font-bold text-gray-500">или распознайте фото в студии</span></span></label><button type="button" onClick={onOpenDictionaryStudio} className="rounded-xl bg-purple-600 px-4 py-2.5 text-sm font-black text-white">OCR и темы</button></div>{isUploadingDictionary && <p className="mt-2 text-xs font-bold text-purple-700">Сохраняю словарь...</p>}</section>}
      <button type="button" onClick={onStartGame} disabled={!canStart} className={`mt-5 w-full rounded-2xl py-4 font-black ${canStart ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{canStart ? `Начать: ${MODE_LABELS[selectedPlayMode]}` : hasPremium ? `Нет слов из ${settings.wordLength} букв` : 'Нужен Premium'}</button>
    </div>
  </ScreenContainer>;
};