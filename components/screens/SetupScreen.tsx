...
const DIFFICULTIES: Array<{ value: DifficultyLevel; short: string }> = [
  ...
  { value: 'B1', short: 'B1' }, { value: 'B2', short: 'B2' }, { value: 'C1', short: 'C1' },
];
...
export const SetupScreen: React.FC<SetupScreenProps> = ({ selectedPlayMode, settings, customDictionaryWords, setupError, isUploadingDictionary, isAuthenticated, userProfile, onOpenDictionaryStudio, onOpenPremium, onSettingsChange, onStartGame, onBack, onLogin }) => {
  ...
  return <ScreenContainer className="max-w-3xl px-3 pb-20 pt-3 sm:px-4">
    ...
    <div className="rounded-[2rem] border-2 border-indigo-50 bg-white p-4 shadow-sm sm:p-6">
      ...
      {settings.dictionarySource === 'builtin' && <section className="mt-4" aria-labelledby="difficulty-title"><h2 id="difficulty-title" className="mb-2 text-xs font-black uppercase tracking-widest text-indigo-400">Сложность</h2><div className="grid grid-cols-3 gap-2 sm:grid-cols-6" role="group" aria-label="Сложность словаря">{DIFFICULTIES.map(level => <button type="button" key={level.value} aria-pressed={settings.difficulty === level.value} onClick={() => updateSetting('difficulty', level.value)} className={`rounded-2xl py-2.5 text-sm font-black ${settings.difficulty === level.value ? 'bg-indigo-600 text-white' : 'border-2 border-indigo-100 text-indigo-700'}`}>{level.short}</button>)}</div></section>}
