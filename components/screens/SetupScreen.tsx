...
export const SetupScreen: React.FC<SetupScreenProps> = ({ selectedPlayMode, settings, customDictionaryWords, setupError, isAuthenticated, userProfile, onSettingsChange, onOpenDictionaryStudio, onStartGame, onBack, onLogin }) => {
  ...
  return <ScreenContainer className="max-w-3xl px-3 pb-20 pt-3 sm:px-4">
    ...
    <div className="rounded-[2rem] border-2 border-indigo-50 bg-white p-4 shadow-sm sm:p-6">
      ...
      {isCustomDictionary && hasPremium && <section className="mt-4 rounded-2xl border-2 border-dashed border-purple-100 bg-purple-50/50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><span className="block font-black text-indigo-950">{customDictionaryWords.length ? 'Словарь готов к игре' : 'Создайте словарь'}</span><span className="block text-xs font-bold text-gray-500">Фото, редактирование, темы и классы — в студии словарей</span></div><button type="button" onClick={onOpenDictionaryStudio} className="rounded-xl bg-purple-600 px-4 py-2.5 text-sm font-black text-white">OCR и темы</button></div></section>}
