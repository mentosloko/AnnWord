...
export const Shop: React.FC<ShopProps> = ({ userProfile, onBuy, onClose, onOpenPetRoom }) => {
  ...
  const firstAffordableTarget = filteredItems.filter(item => !getInventoryQuantity(activeProfile.inventory, item.id)).sort((a, b) => a.price - b.price)[0];
  const coinsNeededForFirst = Math.max(0, (firstAffordableTarget?.price || 0) - activeProfile.coins);
