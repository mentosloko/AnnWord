...
export const Shop: React.FC<ShopProps> = ({ userProfile, onBuy, onClose, onOpenPetRoom }) => {
  ...
  return <div className="mx-auto flex max-w-5xl flex-col p-3 pb-24 sm:p-4">
    ...
    <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div className="overflow-x-auto"><div className="flex w-max gap-2 rounded-2xl bg-indigo-50 p-1" role="tablist" aria-label="Разделы магазина">{VISIBLE_SHOP_TABS.map(tab => <button key={tab} role="tab" aria-selected={activeTab === tab} aria-controls={`shop-panel-${tab}`} disabled={Boolean(buyingId)} onClick={() => setActiveTab(tab)} className={`rounded-xl px-4 py-2 font-bold ${activeTab === tab ? 'bg-white text-indigo-900 shadow-sm' : 'text-indigo-400'}`}>{getTabLabel(tab)}</button>)}</div></div><button type="button" aria-pressed={showOnlyAffordable} onClick={() => setShowOnlyAffordable(value => !value)} className={`rounded-2xl px-4 py-2 text-sm font-black ${showOnlyAffordable ? 'bg-green-600 text-white' : 'border-2 border-green-100 bg-white text-green-700'}`}>{showOnlyAffordable ? 'Показать всё' : `Можно купить (${affordableItems.length})`}</button></div>
    <div id={`shop-panel-${activeTab}`} role="tabpanel" className={`grid gap-3 sm:gap-4 ${activeTab === 'food' ? 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>{filteredItems.map(item => { const locked = activeProfile.pet.level < item.minLevel; const affordable = activeProfile.coins >= item.price; const qty = getInventoryQuantity(activeProfile.inventory, item.id); const owned = qty > 0 && item.type !== 'food' && item.type !== 'mystery'; const imageUrl = getShopImageUrl(item); const canBuy = !locked && affordable && !owned && !buyingId; const compactFood = item.type === 'food'; return <motion.div key={item.id} whileHover={{ y: locked ? 0 : -3 }} className={`relative flex min-w-0 flex-col bg-white shadow-sm ${compactFood ? 'rounded-2xl border border-indigo-100 p-3' : 'rounded-3xl border-2 border-indigo-50 p-5'} ${canBuy ? 'ring-2 ring-green-100' : ''}`}>{locked && <div className="absolute right-2 top-2 rounded-full bg-gray-100 px-2 py-1 text-[10px] font-bold text-gray-500">Ур. {item.minLevel}</div>}{owned && <div className="absolute right-3 top-3 rounded-full bg-green-50 px-2 py-1 text-xs font-bold text-green-700">Есть</div>}{qty > 0 && compactFood && <div className={`absolute right-2 top-2 rounded-full bg-pink-500 px-2 py-1 text-xs font-black text-white ${pulseItemId === item.id ? 'scale-125 animate-bounce' : ''}`}>×{qty}</div>}<div className={`mx-auto flex items-center justify-center bg-indigo-50 ${compactFood ? 'mb-2 h-16 w-16 rounded-xl sm:h-20 sm:w-20' : 'mb-4 h-24 w-24 rounded-2xl'}`}>{imageUrl ? <img src={imageUrl} alt={item.name} className="h-full w-full object-contain" /> : <span className={compactFood ? 'text-4xl' : 'text-5xl'}>🎁</span>}</div><h3 className={`text-center font-bold text-indigo-900 ${compactFood ? 'line-clamp-2 text-sm leading-tight sm:text-base' : 'mb-2 text-xl'}`}>{item.name}</h3>{!compactFood && <p className="min-h-[42px] text-center text-sm text-gray-500">{item.description}</p>}<div className={`text-center font-bold text-indigo-500 ${compactFood ? 'mt-1 text-xs' : 'mt-2 text-xs'}`}>{getItemBenefitText(item)}</div><div className={`flex items-center justify-center gap-1.5 font-bold text-yellow-700 ${compactFood ? 'my-2 text-sm' : 'mb-3 mt-4'}`}><span>{item.price}</span><CoinIcon /></div><button type="button" disabled={!canBuy} onClick={() => handleBuy(item)} className={`mt-auto w-full font-bold ${compactFood ? 'rounded-xl py-2 text-xs sm:text-sm' : 'rounded-2xl py-3'} ${canBuy ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{buyingId === item.id ? 'Сохраняю…' : owned ? 'Уже есть' : locked ? `С уровня ${item.minLevel}` : !affordable ? 'Не хватает монет' : 'Купить'}</button></motion.div>; })}</div>
