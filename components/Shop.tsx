...
export const Shop: React.FC<ShopProps> = ({ userProfile, onBuy, onClose, onOpenPetRoom }) => {
  ...
  const firstTarget = allItems.filter(item => !getInventoryQuantity(activeProfile.inventory, item.id)).sort((a, b) => Math.max(0, a.price - activeProfile.coins) - Math.max(0, b.price - activeProfile.coins) || a.price - b.price)[0];
  const coinsNeededForFirst = Math.max(0, (firstTarget?.price || 0) - activeProfile.coins);
  ...
  return <div className="mx-auto flex max-w-5xl flex-col p-3 pb-24 sm:p-4">
    ...
    {firstTarget && coinsNeededForFirst > 0 ? <div className="mb-4 rounded-3xl border-2 border-amber-100 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-900">Ближайшая цель: «{firstTarget.name}». Не хватает {coinsNeededForFirst} монет. Монеты дают игры и задания питомца.</div> : <div className="mb-4 rounded-3xl border-2 border-green-100 bg-green-50 px-4 py-3 text-sm font-bold text-green-800">Есть доступные покупки: начните с карточек наверху списка.</div>}
