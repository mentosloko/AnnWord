...
export const Shop: React.FC<ShopProps> = ({ userProfile, onBuy, onClose, onOpenPetRoom }) => {
  ...
  return <div className="mx-auto flex max-w-5xl flex-col p-3 pb-24 sm:p-4">
    ...
    <div id={`shop-panel-${activeTab}`} role="tabpanel" className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">{filteredItems.map(item => { const locked = activeProfile.pet.level < item.minLevel; const affordable = activeProfile.coins >= item.price; const qty = getInventoryQuantity(activeProfile.inventory, item.id); const owned = qty > 0 && item.type !== 'food' && item.type !== 'mystery'; const imageUrl = getShopImageUrl(item); const canBuy = !locked && affordable && !owned && !buyingId; const compactCard = item.type === 'food' || item.type === 'accessory'; return <motion.div key={item.id} whileHover={{ y: locked ? 0 : -3 }} className={`relative flex min-w-0 flex-col bg-white shadow-sm ${compactCard ? 'rounded-2xl border border-indigo-100 p-3' : 'rounded-3xl border-2 border-indigo-50 p-5'} ${canBuy ? 'ring-2 ring-green-100' : ''}`}>{locked && <div className="absolute right-2 top-2 rounded-full bg-gray-100 px-2 py-1 text-[10px] font-bold text-gray-500">Ур. {item.minLevel}</div>}{owned && <div className="absolute right-2 top-2 rounded-full bg-green-50 px-2 py-1 text-[10px] font-bold text-green-700 sm:text-xs">Есть</div>}{qty > 0 && item.type === 'food' && <div className={`absolute right-2 top-2 rounded-full bg-pink-500 px-2 py-1 text-xs font-black text-white ${pulseItemId === item.id ? 'scale-125 animate-bounce' : ''}`}>×{qty}</div>}<div className={`mx-auto flex items-center justify-center bg-indigo-50 ${compactCard ? 'mb-2 h-16 w-16 rounded-xl sm:h-20 sm:w-20' : 'mb-4 h-24 w-24 rounded-2xl'}`}>{imageUrl ? <img src={imageUrl} alt={item.name} className="h-full w-full object-contain" /> : <span className={compactCard ? 'text-4xl' : 'text-5xl'}>🎁</span>}</div><h3 className={`text-center font-bold text-indigo-900 ${compactCard ? 'line-clamp-2 text-sm leading-tight sm:text-base' : 'mb-2 text-xl'}`}>{item.name}</h3>{!compactCard && <p className="min-h-[42px] text-center text-sm text-gray-500">{item.description}</p>}<div className={`text-center font-bold text-indigo-500 ${compactCard ? 'mt-1 text-xs' : 'mt-2 text-xs'}`}>{getItemBenefitText(item)}</div><div className={`flex items-center justify-center gap-1.5 font-bold text-yellow-700 ${compactCard ? 'my-2 text-sm' : 'mb-3 mt-4'}`}><span>{item.price}</span><CoinIcon /></div><button type="button" disabled={!canBuy} onClick={() => handleBuy(item)} className={`mt-auto w-full font-bold ${compactCard ? 'rounded-xl py-2 text-xs sm:text-sm' : 'rounded-2xl py-3'} ${canBuy ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{buyingId === item.id ? 'Сохраняю…' : owned ? 'Уже есть' : locked ? `С уровня ${item.minLevel}` : !affordable ? 'Не хватает монет' : 'Купить'}</button></motion.div>; })}</div>
