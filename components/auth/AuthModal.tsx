...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, mode, email, password, error, isLoading, onClose, onModeChange, onEmailChange, onPasswordChange, onSubmit, onYandexLogin }) => {
  ...
  const title = recoveryMode ? 'Восстановление пароля' : mode === 'login' ? 'Войти в AnnWord' : 'Создать аккаунт родителя';
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-indigo-950/55 p-3 backdrop-blur-sm" role="presentation">
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="auth-modal-title" tabIndex={-1} className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-y-auto rounded-[1.75rem] bg-white p-5 shadow-2xl outline-none sm:p-6">
      <div className="flex items-start justify-between gap-4"><h2 id="auth-modal-title" className="text-2xl font-bold text-indigo-950">{title}</h2><button type="button" onClick={onClose} aria-label="Закрыть окно" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-xl font-bold text-indigo-600">×</button></div>
      {visibleMessage && <StableStatusSlot message={visibleMessage} tone={visibleTone} role={visibleTone === 'error' ? 'alert' : 'status'} className="mt-3" minHeightClassName="min-h-0" />}
      {recoveryMode ? <form onSubmit={requestRecovery} className="mt-2 space-y-4">
        ...
        <button type="button" disabled={isLoading} onClick={() => onModeChange(mode === 'login' ? 'register' : 'login')} className="mt-4 w-full text-sm font-bold text-indigo-700">{mode === 'login' ? 'Нет аккаунта? Зарегистрироваться' : 'Уже есть аккаунт? Войти'}</button>
      </>}
    </div>
  </div>;
};