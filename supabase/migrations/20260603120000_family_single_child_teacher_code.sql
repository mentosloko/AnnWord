-- Family MVP: one child plays through a parent's authenticated account.
-- Teachers attach to that learner using a share code and assign saved collections.
...
begin
  if auth.uid() is null then raise exception 'Unauthorized'; end if;
  select * into v_profile from public.profiles where id = auth.uid() for update;
  if v_profile.role <> 'teacher' and v_profile.role <> 'admin' and not public.has_active_premium(auth.uid()) then raise exception 'Premium required'; end if;
  if p_source = 'ocr' and v_profile.role = 'teacher' then raise exception 'Teacher OCR unavailable'; end if;
  ...
  perform set_config('annword.dictionary_write', 'allowed', true);
  update public.profiles set dictionary_collections = coalesce(dictionary_collections, '[]'::jsonb) || jsonb_build_array(v_collection), custom_dictionary_en = case when role <> 'teacher' then to_jsonb(v_words) else custom_dictionary_en end where id = auth.uid();
  return v_collection;
end;
$$;
...
begin
  if new.custom_dictionary_en is distinct from old.custom_dictionary_en
