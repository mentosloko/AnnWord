...
const AppV2: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (userProfile.accountMode === 'parent' && !userProfile.childDisplayName) { if (route !== 'family_setup') setRoute('family_setup'); return; }
    if (route !== 'character_onboarding' && !userProfile.pet.characterOnboarded) setRoute('character_onboarding');
  }, [bootstrapStatus, isAuthenticated, isTeacher, route, setRoute, userProfile.accountMode, userProfile.childDisplayName, userProfile.pet.characterOnboarded, userProfile.role]);

  const loadDailyQuest = useCallback(async () => {
    if (bootstrapStatus !== 'ready' || !isAuthenticated || isTeacher || !userProfile.pet.characterOnboarded) { setDailyQuest(null); return; }
    try { setDailyQuest(await dailyQuestService.getTodayQuest()); } catch (error) { console.error('Failed to load daily quest', error); }
  }, [bootstrapStatus, isAuthenticated, isTeacher, userProfile.pet.characterOnboarded]);
  useEffect(() => {
    void loadDailyQuest();
    if (bootstrapStatus !== 'ready' || !isAuthenticated || isTeacher || !userProfile.pet.characterOnboarded || typeof window === 'undefined' || typeof document === 'undefined') return;
  ...
  }, [bootstrapStatus, isAuthenticated, isTeacher, loadDailyQuest, userProfile.pet.characterOnboarded]);
  ...
  const submitDailyQuestResult = useCallback(async (input: GameRewardInput) => { if (!isAuthenticated || isTeacher || !userProfile.pet.characterOnboarded) return; try { const result = await dailyQuestService.submitGameResult(input); setDailyQuest(result.quest); if (result.profile) setUserProfile(result.profile); if (result.reward) setDailyQuestReward(result.reward); } catch (error) { console.error('Failed to apply daily quest result', error); } }, [isAuthenticated, isTeacher, setUserProfile, userProfile.pet.characterOnboarded]);
  ...
  const handleSelectAccountMode = useCallback(async (mode: AccountMode) => {
    ...
    setRoute(mode === 'teacher' ? 'adult_room' : mode === 'parent' ? 'family_setup' : userProfile.pet.characterOnboarded ? 'landing' : 'character_onboarding');
  }, [setRoute, setUserProfile, userProfile.pet.characterOnboarded]);
