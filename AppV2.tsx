...
const isRewardStatWin = (input: GameRewardInput): boolean => {
  ...
  if (input.type === 'sprint' || input.type === 'anagram') return Math.max(0, Math.round(input.guessedWords || 0)) > 0;
...
const addRewardToStats = (stats: UserStats, input: GameRewardInput): UserStats => {
  ...
  const next: UserStats = { ...stats, wordsGuessed: { ...stats.wordsGuessed }, wordsToReview: { ...(stats.wordsToReview || {}) }, wordPerformance: stats.wordPerformance ? { ...stats.wordPerformance } : undefined };
  next.gamesPlayed += 1;
...
const AppV2: React.FC = () => {
  ...
  const handleTestUnlockPremium = useCallback(() => { setUserProfile(previous => ({ ...previous, subscriptionTier: 'premium', premiumExpiresAt: undefined, featureFlags: { ...(previous.featureFlags || {}), premiumDictionaries: true } })); setSettings(previous => ({ ...previous, dictionarySource: 'premium', useCustomDictionary: false, activePremiumDictionaryId: previous.activePremiumDictionaryId || getDefaultPremiumDictionaryId() })); setRoute('setup'); }, [setRoute, setSettings, setUserProfile]);
