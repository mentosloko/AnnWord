...
const TEACHER_ROUTES: ViewState[] = ['adult_room', 'dictionary_studio', 'admin'];
...
const AppV2: React.FC = () => {
  ...
  const setRoute = useCallback((nextRoute: ViewState) => setRouteState(previous => { if (previous !== nextRoute) analyticsService.trackEvent({ userId: currentUserId, eventType: 'navigation', eventName: 'route_changed', route: nextRoute, payload: { previousRoute: previous, nextRoute } }); return nextRoute; }), [currentUserId]);
  ...
  useEffect(() => { if (bootstrapStatus === 'ready') preloadAppAssetsForProfile(userProfile); }, [bootstrapStatus, userProfile.pet.type, userProfile.pet.characterOnboarded]);
  useEffect(() => {
    if (bootstrapStatus !== 'ready' || !isAuthenticated) return;
    if (userProfile.role === 'teacher') { if (!TEACHER_ROUTES.includes(route)) setRoute('adult_room'); return; }
    if (userProfile.role !== 'admin' && !userProfile.childDisplayName) { if (route !== 'family_setup') setRoute('family_setup'); return; }
    if (!userProfile.pet.characterOnboarded && route !== 'character_onboarding') setRoute('character_onboarding');
  }, [bootstrapStatus, isAuthenticated, route, setRoute, userProfile.childDisplayName, userProfile.pet.characterOnboarded, userProfile.role]);

  const loadDailyQuest = useCallback(async () => {
    if (bootstrapStatus !== 'ready' || !isAuthenticated || userProfile.role === 'teacher' || !userProfile.pet.characterOnboarded) { setDailyQuest(null); return; }
    try { setDailyQuest(await dailyQuestService.getTodayQuest()); } catch (error) { console.error('Failed to load daily quest', error); }
  }, [bootstrapStatus, isAuthenticated, userProfile.pet.characterOnboarded, userProfile.role]);
  useEffect(() => {
    void loadDailyQuest();
    if (bootstrapStatus !== 'ready' || !isAuthenticated || userProfile.role === 'teacher' || !userProfile.pet.characterOnboarded || typeof window === 'undefined' || typeof document === 'undefined') return;
  ...
  }, [bootstrapStatus, isAuthenticated, loadDailyQuest, userProfile.pet.characterOnboarded, userProfile.role]);
  ...
  const submitDailyQuestResult = useCallback(async (input: GameRewardInput) => { if (!isAuthenticated || userProfile.role === 'teacher' || !userProfile.pet.characterOnboarded) return; try { const result = await dailyQuestService.submitGameResult(input); setDailyQuest(result.quest); if (result.profile) setUserProfile(result.profile); if (result.reward) setDailyQuestReward(result.reward); } catch (error) { console.error('Failed to apply daily quest result', error); } }, [isAuthenticated, setUserProfile, userProfile.pet.characterOnboarded, userProfile.role]);
  const updateClassicStats = useCallback(async (won: boolean, word: string, coinsAdjustment = 0) => { const nextStats: UserStats = { ...userProfile.stats, wordsGuessed: { ...userProfile.stats.wordsGuessed }, wordsToReview: { ...(userProfile.stats.wordsToReview || {}) } }; nextStats.gamesPlayed += 1; if (won) { nextStats.gamesWon += 1; nextStats.wordsGuessed[word] = (nextStats.wordsGuessed[word] || 0) + 1; } const input: GameRewardInput = { type: 'wordle', won, coinsAdjustment }; const event = analyticsService.createEvent({ userId: currentUserId, eventType: 'game', eventName: 'game_finished', gameType: 'wordle', route: 'game', payload: { won, word, coinsAdjustment, wordLength: settings.wordLength, dictionarySource: settings.dictionarySource, difficulty: settings.difficulty } }); await profileEconomy.applyGameReward(input, { stats: nextStats, analyticsEvents: [event] }); }, [currentUserId, profileEconomy, settings.dictionarySource, settings.difficulty, settings.wordLength, userProfile.stats]);
  ...
  const handleSaveDictionary = useCallback(async (draft: PremiumDictionaryDraft) => { const collection = await premiumDictionaryService.saveCollection(draft); setUserProfile(previous => ({ ...previous, customDictionaryEn: previous.role === 'teacher' ? previous.customDictionaryEn : collection.words, dictionaryCollections: [collection, ...(previous.dictionaryCollections || []).filter(item => item.id !== collection.id)] })); if (userProfile.role !== 'teacher') setDictionarySource('custom'); }, [setDictionarySource, setUserProfile, userProfile.role]);
  ...
  const handleChildSetupComplete = useCallback((result: ChildSetupResult) => { setUserProfile(previous => ({ ...previous, role: 'parent', childDisplayName: result.childName, childShareCode: result.childShareCode, childSlotsLimit: result.childSlotsLimit, featureFlags: { ...(previous.featureFlags || {}), adultRoom: true } })); setRoute('character_onboarding'); }, [setRoute, setUserProfile]);
  const handleGameReward = useCallback(async (input: GameRewardInput) => { const event = analyticsService.createEvent({ userId: currentUserId, eventType: 'game', eventName: 'game_finished', gameType: input.type, route: input.type === 'other' ? route : input.type, payload: { ...input, wordLength: settings.wordLength, dictionarySource: settings.dictionarySource, difficulty: settings.difficulty } }); await profileEconomy.applyGameReward(input, { analyticsEvents: [event] }); await submitDailyQuestResult(input); }, [currentUserId, profileEconomy, route, settings.dictionarySource, settings.difficulty, settings.wordLength, submitDailyQuestResult]);
  const handleRecordReviewWord = useCallback(async (word: string) => { const value = word.trim().toUpperCase(); if (!value) return; const stats: UserStats = { ...userProfile.stats, wordsGuessed: { ...userProfile.stats.wordsGuessed }, wordsToReview: { ...(userProfile.stats.wordsToReview || {}) } }; stats.wordsToReview![value] = (stats.wordsToReview![value] || 0) + 1; await profileEconomy.updateStats(stats); }, [profileEconomy, userProfile.stats]);
