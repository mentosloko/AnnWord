...
export const toCustomEnrichedWords = (words: string[] = []): EnrichedWord[] =>
  normalizeCustomDictionary(words).map(word => ({
    word,
    translation: '',
    level: CUSTOM_LEVEL,
  }));
...
export const getBuiltinSecretWordPool = (selection: Pick<DictionarySelection, 'wordLength' | 'difficulty'>): EnrichedWord[] => {
  ...
  if (selection.difficulty !== 'ALL') {
    pool = pool.filter(item => item.level === selection.difficulty);
  }
...
export const getSecretWordPool = (selection: DictionarySelection): EnrichedWord[] => {
  if (selection.source === 'custom' && normalizeCustomDictionary(selection.customDictionaryEn).length > 0) {
    return getCustomSecretWordPool(selection.customDictionaryEn, selection.wordLength);
  }
...
export const getValidationPool = (selection: Pick<DictionarySelection, 'wordLength' | 'customDictionaryEn'>): string[] => {
  const builtin = ALL_WORDS_EN
    .map(normalizeWord)
    .filter(word => word.length === selection.wordLength);

  const custom = normalizeCustomDictionary(selection.customDictionaryEn)
    .filter(word => word.length === selection.wordLength);
...
export const buildDictionaryPools = (selection: DictionarySelection): DictionaryPools => {
  ...
  const effectiveSource: DictionarySource =
    selection.source === 'custom' && customDictionaryNormalized.length > 0 ? 'custom' : 'builtin';

  return {
    secretWordPool: getSecretWordPool(selection),
    validationPool: getValidationPool(selection),
    customDictionaryNormalized,
    dictionarySourceUsed: effectiveSource,
  };
};
...
export const getDictionaryEmptyStateMessage = (selection: DictionarySelection): string => {
  if (selection.source === 'custom') {
    return `В вашем словаре нет слов длиной ${selection.wordLength}.`;
  }
