...
export const getTotalXpForLevel = (level: number) => {
  ...
  if (normalizedLevel <= T.length) return T[normalizedLevel - 1];
  const extraLevels = normalizedLevel - T.length;
  return T[T.length - 1] + extraLevels * 2000 + extraLevels * (extraLevels - 1) * 150;
};
...
export const calculateGameReward = (input: GameRewardInput): GameRewardResult => {
  ...
  if (input.type === 'anagram') {
    const guessedWords = Math.max(0, Math.round(input.guessedWords || 0));
    const xp = guessedWords ? Math.min(25, guessedWords * 5) : 5;
    return reward(xp, C.anagram.success, 'Anagram done');
  }
  if (input.type === 'memory') {
    const clicks = Math.max(0, Math.round(input.clicks || 0));
    const xp = clicks > 0 && clicks <= 12 ? 30 : clicks <= 16 ? 25 : clicks <= 20 ? 20 : clicks <= 24 ? 15 : clicks > 24 ? 10 : 8;
...
export const applyGameRewardToCharacter = (pet: PetState, reward: GameRewardResult): CharacterProgressResult => {
  const previousLevel = deriveCharacterLevel(pet.xp || 0);
  const xp = Math.max(0, Math.round(pet.xp || 0)) + Math.max(0, Math.round(reward.xp || 0));
  const newLevel = deriveCharacterLevel(xp);
  const moodScore = normalizeMoodScore(pet);
  const updatedPet = { ...pet, xp, level: newLevel, stage: st(newLevel), moodScore, mood: deriveMoodFromScore(moodScore) };
  return { pet: updatedPet, previousLevel, newLevel, previousStage: st(previousLevel), newStage: st(newLevel), leveledUp: newLevel > previousLevel, stagedUp: st(newLevel) !== st(previousLevel) };
};

export const applyTreatMood = (pet: PetState, delta: number, _cap?: number): PetState => {
  const moodScore = Math.min(100, normalizeMoodScore(pet) + Math.max(0, Math.round(delta || 0)));
  return { ...pet, moodScore, mood: deriveMoodFromScore(moodScore) };
};
