...
export interface PetState { name: string; type: string; level: number; mood: CharacterMood; xp: number; moodScore?: number; moodUpdatedAt?: string; stage?: CharacterStage; characterOnboarded?: boolean; hunger?: number; energy?: number; equippedAccessories: string[]; activeHomeItemId?: string; activeWorldId?: PetWorldId; activeWorldDate?: string; dailyStreak?: number; lastDailyActivityDate?: string; earnedStickerIds?: string[]; requestedTreatId?: string; }
...
export interface CustomDictionaryCollection { id: string; title: string; source: 'manual' | 'ocr' | 'class' | 'topic'; words: string[]; wordTranslations?: Record<string, string>; classLabel?: string; theme?: string; createdAt?: string; }
export interface ActiveWordSource { source: DictionarySource; difficulty: DifficultyLevel; premiumDictionaryId?: string; spotlightGrade?: number; spotlightSectionId?: string; spotlightSectionIds?: string[]; updatedAt?: string; }
export interface UserProfile { username: string; role?: AccountRole; accountMode?: AccountMode; subscriptionTier?: SubscriptionTier; premiumExpiresAt?: string; kidsTrialStartedAt?: string; kidsTrialExpiresAt?: string; childDisplayName?: string; childShareCode?: string; childSlotsLimit?: number; featureFlags?: FeatureFlags; activeWordSource?: ActiveWordSource; customDictionaryEn: string[]; assignedWords?: string[]; assignedWordTranslations?: Record<string, string>; dictionaryCollections?: CustomDictionaryCollection[]; managedLearners?: ManagedLearner[]; weeklyReportEmail?: string; petWardrobeAutoRemoved?: boolean; stats: UserStats; pet: PetState; coins: number; inventory: InventoryItem[]; }
export type DailyQuestKind = 'wordle_four' | 'sprint_twelve' | 'memory_sixteen' | 'hangman_clean' | 'all_five_games';
export type DailyQuestCompletedMode = 'letter_square' | 'sprint' | 'anagram' | 'memory' | 'hangman';
export interface DailyQuestState { questDate: string; kind: DailyQuestKind; title: string; description: string; progressLabel: string; completedModes?: DailyQuestCompletedMode[]; completed: boolean; completedAt?: string | null; rewardItemId?: string | null; rewardWorldId?: PetWorldId | null; }
...
export interface GameSettings { wordLength: WordLength; useCustomDictionary: boolean; dictionarySource: DictionarySource; difficulty: DifficultyLevel; username: string; activePremiumDictionaryId?: string; activeSpotlightGrade?: number; activeSpotlightSectionId?: string; activeSpotlightSectionIds?: string[]; activeWordSourceUpdatedAt?: string; }
