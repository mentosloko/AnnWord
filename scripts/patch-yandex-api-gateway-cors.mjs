...
if (matches.length > 1) {
  throw new Error(`Expected at most one wildcard CORS origin, found ${matches.length}. Refusing to change the gateway.`);
}

if (matches.length === 0) {
  const expectedOrigins = origins.map((origin) => new RegExp(`^[ \\t]+- ${origin.replace(/[.*+?^\${}()|[\\]\\]/g, "\\\\const wildcardOrigin = /^([ \t]*)origin:\s*(?:'\*'|"\*"|\*)[ \t]*$/gm;
const matches = [...source.matchAll(wildcardOrigin)];
