...
const baseProfile: UserProfile = {
  ...
  pet: { name: 'Owl', type: 'Owl', level: 5, mood: 'neutral', xp: 0, hunger: 50, energy: 100, equippedAccessories: [] },
  coins: 100,
  inventory: [],
};
...
assert(SHOP_ITEMS.length >= 5, 'shop catalog must include multiple item categories');
assert(getShopItemsByType('food').every(item => item.type === 'food'), 'food tab must only include food items');
...
assert(purchaseOne.profile!.coins === 90, 'purchase must subtract item price once');
...
assert(purchaseTwo.profile!.coins === 80, 'second purchase must subtract price once');
...
assert(usedApple.ok && usedApple.profile, 'using food must succeed');
assert(usedApple.profile!.pet.hunger === 70, 'using food must increase hunger by 20');
assert(getInventoryQuantity(usedApple.profile!.inventory, 'apple') === 1, 'using food must decrement quantity');
...
const lockedItem = getShopItemById('dragon') as ShopItem;
