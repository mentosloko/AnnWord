...
export const useAuthProfile = () => {
  ...
  const loadProfileForUser = useCallback(async (user: User): Promise<UserProfile> => { const { userService } = await import('../services/userService'); const profile = await userService.getOrCreateProfile(user.id, user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || 'Пользователь', user.email || undefined); profileCache.write(profile, user.id); setUserProfileState(profile); setSettings(prev => ({ ...prev, ...readStoredSettings(user.id), username: profile.username })); return profile; }, []);
  const refreshCurrentProfile = useCallback(async (): Promise<UserProfile | null> => { const user = currentUser || (await authService.getInitialSession()).user; if (!user) return null; currentUserIdRef.current = user.id; setCachedUserId(user.id); setCurrentUser(user); return loadProfileForUser(user); }, [currentUser, loadProfileForUser]);
  ...
  return { bootstrapStatus, bootstrapError, isRestoringSession, settings, setSettings, userProfile, setUserProfile, currentUser, cachedUserId, isAuthenticated: Boolean(currentUser) || Boolean(cachedUserId), authMode, setAuthMode, tempUsername, setTempUsername, tempPassword, setTempPassword, authError, isAuthLoading, setAuthError, openLoginMode, openRegisterMode, submitEmailAuth, loginWithYandex, logout, refreshCurrentProfile };
};