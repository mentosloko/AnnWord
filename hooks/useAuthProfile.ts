...
export const useAuthProfile = () => {
  ...
  const submitEmailAuth = useCallback(async () => { if (!tempUsername.trim() || (authMode === 'login' && !tempPassword.trim())) { setAuthError('Заполните все поля'); return false; } setIsAuthLoading(true); setAuthError(null); let profileWillLoadFromAuthEvent = false; try { if (authMode === 'login') { await authService.signInWithEmail(tempUsername, tempPassword); profileWillLoadFromAuthEvent = true; } else { const result = await authService.signUpWithEmail(tempUsername, tempPassword); if (result.needsEmailConfirmation) setAuthError('Мы отправили ссылку для входа на вашу электронную почту. Откройте письмо и перейдите по ссылке.'); else profileWillLoadFromAuthEvent = true; } return true; } catch (error: unknown) { setAuthError(getAuthErrorMessage(error, 'Не удалось войти. Попробуйте ещё раз.')); return false; } finally { if (!profileWillLoadFromAuthEvent) setIsAuthLoading(false); } }, [authMode, tempPassword, tempUsername]);
